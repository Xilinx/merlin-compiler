...
#pragma ACCEL wrapper variable=a port=a depth=10000 max_depth=10000 data_type=int io=RW copy=false
{
//    if (a != 0) {
...
{
    if (a != 0) {
//        opencl_read_buffer(__test_kernel__a_buffer,commands[0],0 * sizeof(int ),((int *)a),((unsigned long )10000) * sizeof(int ));
        int i;
        for(i=0; i<burst_number; i++) {
            opencl_read_buffer(__test_kernel__a_buffer,commands[0],0 * sizeof(int ),((int *)a),(burst_length) * sizeof(int ));
        }
    }
     else {
      printf("Error! The external memory pointed by 'a' is invalid.\n");
