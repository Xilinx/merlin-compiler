...
int cmost_free(void * a) { cmost_free_1d(a); return 1;}
int cmost_dump(void * a, const char * filename) { cmost_dump_1d(a, filename); return 1;} 
...
void cmost_load_file(void * buffer, const char * file_name, size_t size)
{
	...
	{
        size_t i;
        for (i = 0; i < size/4; i++) ((int*)buffer)[i] = i;
	}
    else if (strcmp(file_name, "&") == 0)
	{
        size_t i;
        for (i = 0; i < size/4; i++) ((int*)buffer)[i] = rand();
	}
    else 
    {
        ...
        FILE * fp;
        char fullname[1024];
        if (file_name[0] != '/') { strcpy(fullname, data_file_prefix);
                                   strcat(fullname, file_name);
        }
        else strcpy(fullname, file_name);
        printf("Open file -- %s\n", fullname);
...
int cmost_malloc_1d(void ** a, const char * filename, int unit_size, size_t d0 )
{
	...
	/* allocate 3 more words to store dimensions d0, d1, d2, at the start of the container */
	assert(sizeof(data_type) == sizeof(int));	/* currently only support when data and int have same size */
	assert(unit_size == sizeof(int));	/* currently only support when data and int have same size */
...
//int cmost_malloc_2d(void *** a, char * filename, int unit_size, size_t d0, size_t d1 )
//{
//	int i, j;
//	FILE* fp;
//
...
//	assert(sizeof(data_type) == sizeof(int));	/* currently only support when data and int have same size */
//	assert(unit_size == sizeof(int));	/* currently only support when data and int have same size */
//	data_type * container = (data_type *)malloc((2+(d0*d1) )* sizeof (data_type));
//
//  assert(container);
//
//	if (container != 0) {
...
//}
//
//int cmost_malloc_3d(void **** a, char * filename, int unit_size, size_t d0, size_t d1, size_t d2 )
//{
//	int i, j;
//	FILE* fp;
//
...
//	assert(sizeof(data_type) == sizeof(int));	/* currently only support when data and int have same size */
//	assert(unit_size == sizeof(int));	/* currently only support when data and int have same size */
//	data_type * container = (data_type *)malloc((3 + (d0*d1*d2)) * sizeof (data_type));
//
//  assert(container);
//
//	if (container != 0) {
