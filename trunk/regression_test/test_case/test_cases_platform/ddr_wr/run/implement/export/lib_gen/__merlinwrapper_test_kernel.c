...
#pragma ACCEL wrapper variable=a port=a depth=100000000 max_depth=100000000 data_type=int io=RW copy=false
//  static  __m_a;
{
//    if (a != 0) {
////       __test_kernel__a_buffer;
//      opencl_write_buffer(__test_kernel__a_buffer,commands[0],0 * sizeof(int ),((int *)a),((unsigned long )100000000) * sizeof(int ));
//    }
//     else {
...
{
    if (a != 0) {
      opencl_read_buffer(__test_kernel__a_buffer,commands[0],0 * sizeof(int ),((int *)a),((unsigned long )100) * sizeof(int ));
    }
     else {
      printf("Error! The external memory pointed by 'a' is invalid.\n");
      exit(1);
