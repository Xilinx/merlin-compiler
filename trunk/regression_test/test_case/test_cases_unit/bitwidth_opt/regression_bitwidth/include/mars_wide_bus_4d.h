...
#define memcpy_wide_bus(access, type, size1, size2, size3, bus) instant_size(access, type, size1, size2, size3, bus)
#define instant_size(access, type, size1, size2, size3, bus) memcpy_wide_bus_##access##_##type##_4d_##size1##_##size2##_##size3##_##bus


static void
memcpy_wide_bus(read, char, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(char a_buf[][SIZE_1][SIZE_2][SIZE_3], 
...
#pragma HLS inline self
  ...
  if ((SIZE_3 % num_elements) == 0 && (index1_offset % num_elements) == 0) {
  ...
  else if ((T_SIZE_3 % num_elements) == 0 && 
           (num_elements % SIZE_3) == 0 &&
      ((index2_offset * SIZE_3) % num_elements) == 0 &&
      !index1_offset) {
    aligned = 2;
    index2 = index2_offset * SIZE_3 / num_elements;
  }
  else if ((T_SIZE % num_elements) == 0 && 
           (num_elements % T_SIZE_3) == 0 &&
    ...
    index3 = index3_offset *T_SIZE_3 / num_elements;
           ...
           (index4_offset * T_SIZE % num_elements) == 0 &&
           !index3_offset && !index2_offset && !index1_offset)  {
    aligned = -1;
    index4 = index4_offset * T_SIZE  / num_elements;
  }
  const size_t index_offset = T_SIZE * index4_offset + T_SIZE_3 * index3_offset +
...
static void
memcpy_wide_bus(write, char, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(MARS_WIDE_BUS_TYPE *c, 
                             char c_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(read, short, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(short a_buf[][SIZE_1][SIZE_2][SIZE_3], 
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(write, short, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(MARS_WIDE_BUS_TYPE *c, 
                             short c_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(read, int, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(int a_buf[][SIZE_1][SIZE_2][SIZE_3], 
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(write, int, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(MARS_WIDE_BUS_TYPE *c, 
                             int c_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(read, long, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(long a_buf[][SIZE_1][SIZE_2][SIZE_3], 
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(write, long, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(MARS_WIDE_BUS_TYPE *c, 
                             long c_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(read, long_long, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(long long a_buf[][SIZE_1][SIZE_2][SIZE_3], 
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(write, long_long, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(MARS_WIDE_BUS_TYPE *c, 
                             long long c_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
#pragma HLS pipeline
    if (head_align == 0 && aligned) {
      if (aligned == 1) {
        for (j = 0; j < num_elements; ++j) {
          long long val = c_buf[index4][index3][index2][index1 * num_elements + j];
          tmp3(c[i + start], data_width, (j * data_width), val);
        }
        index1++;
        if (index1 == bound1) {
...
static void memcpy_wide_bus(read, float, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(
    float a_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
      ...
      if (aligned == 1) {
        for (j = 0; j < num_elements; ++j) {
          if (i == end - start - 1 && j > tail_align)
            ...
            a_buf[index4][index3][index2][index1 * num_elements + j] = *(float*)(&raw_bits);
        }
        index1++;
        if (index1 == bound1) {
          index1 = 0;
        ...
        for (j0 = 0; j0 < bound; ++j0) {
          for (j1 = 0; j1 < SIZE_3; ++j1) {
            ...
            if (i == end - start - 1 && j > tail_align)
              continue;
            int raw_bits = tmp2(a[i + start], data_width, (j * data_width));
            a_buf[index4][index3][index2 * bound + j0][j1] = *(float*)(&raw_bits);
          }
        }
        ++index2;
        if (index2 == bound2) {
          ...
          for (j1 = 0; j1 < SIZE_2; ++j1) {
            for (j2 = 0; j2 < SIZE_3; ++j2) {
              ...
              if (i == end - start - 1 && j > tail_align)
                continue;
              int raw_bits = tmp2(a[i + start], data_width, (j * data_width));
              a_buf[index4][index3 * bound + j0][j1][j2] = *(float*)(&raw_bits);
            }
          }
        }
        ++index3;
...
static void memcpy_wide_bus(write, float, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(MARS_WIDE_BUS_TYPE *c,
                                         float c_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
...
static void
memcpy_wide_bus(read, double, SIZE_1, SIZE_2, SIZE_3, LARGE_BUS)(double a_buf[][SIZE_1][SIZE_2][SIZE_3],
                                                   size_t index4_offset,
                                                   size_t index3_offset,
                                                   size_t index2_offset,
                                                   size_t index1_offset,
      ...
      if (aligned == 1) {
        for (j = 0; j < num_elements; ++j) {
          if (i == end - start - 1 && j > tail_align)
            ...
            a_buf[index4][index3][index2][index1 * num_elements + j] = *(double*)(&raw_bits);
        }
        index1++;
        if (index1 == bound1) {
          index1 = 0;
        ...
        for (j0 = 0; j0 < bound; ++j0) {
          for (j1 = 0; j1 < SIZE_3; ++j1) {
            ...
            if (i == end - start - 1 && j > tail_align)
              continue;
            long long raw_bits = tmp2(a[i + start], data_width, (j * data_width));
            a_buf[index4][index3][index2 * bound + j0][j1] = *(double*)(&raw_bits);
          }
        }
        ++index2;
        if (index2 == bound2) {
          ...
          for (j1 = 0; j1 < SIZE_2; ++j1) {
            for (j2 = 0; j2 < SIZE_3; ++j2) {
              ...
              if (i == end - start - 1 && j > tail_align)
                continue;
              long long raw_bits = tmp2(a[i + start], data_width, (j * data_width));
              a_buf[index4][index3 * bound + j0][j1][j2] = *(double*)(&raw_bits);
            }
          }
        }
        ++index3;
