...
#define instant_size(access, type, bus) memcpy_wide_bus_##access##_##type##_##bus
#define memcpy_wide_bus_single(access, type, bus) instant_single_size(access, type, bus)
#define instant_single_size(access, type, bus) memcpy_wide_bus_single_##access##_##type##_##bus
...
#pragma HLS inline self
  ...
  char ret = tmp2(a[start], data_width,
                 (head_align * data_width));
  return ret;
}

static void memcpy_wide_bus_single(write, char, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c,
                                                    char c_buf,
                                                    size_t offset_byte) {
#pragma HLS inline self
  const size_t data_width = sizeof(char);
  ...
  size_t head_align = offset & (num_elements - 1);
  size_t start = offset / num_elements;
  tmp3(c[start], data_width, (head_align * data_width),
       c_buf);
  return;
}

static short memcpy_wide_bus_single(read, short, LARGE_BUS)(MARS_WIDE_BUS_TYPE *a,
                                                  size_t offset_byte) {
#pragma HLS inline self
  ...
  size_t head_align = offset & (num_elements - 1);
  size_t start = offset / num_elements;
  short ret = tmp2(a[start], data_width,
                 (head_align * data_width));
  return ret;
}

static void memcpy_wide_bus_single(write, short, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c,
                                                    short c_buf,
                                                    size_t offset_byte) {
#pragma HLS inline self
  const size_t data_width = sizeof(short);
...
#pragma HLS inline self
  ...
  size_t head_align = offset & (num_elements - 1);
  size_t start = offset / num_elements;
  int ret = tmp2(a[start], data_width,
                 (head_align * data_width));
  return ret;
}
...
#pragma HLS inline self
  ...
  size_t head_align = offset & (num_elements - 1);
  size_t start = offset / num_elements;
  long ret = tmp2(a[start], data_width,
                 (head_align * data_width));
  return ret;
}

static void memcpy_wide_bus_single(write, long, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c,
                                                    long c_buf,
                                                    size_t offset_byte) {
#pragma HLS inline self
  const size_t data_width = sizeof(long);
...
static long long memcpy_wide_bus_single(read, long_long, LARGE_BUS)(MARS_WIDE_BUS_TYPE *a,
                                                  size_t offset_byte) {
#pragma HLS inline self
  ...
  size_t head_align = offset & (num_elements - 1);
  size_t start = offset / num_elements;
  long long ret = tmp2(a[start], data_width,
                 (head_align * data_width));
  return ret;
}

static void memcpy_wide_bus_single(write, long_long, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c,
                                                    long long c_buf,
                                                    size_t offset_byte) {
#pragma HLS inline self
  const size_t data_width = sizeof(long long);
...
static float
memcpy_wide_bus_single(read, float, LARGE_BUS) (MARS_WIDE_BUS_TYPE *a,
                                  size_t offset_byte) {
#pragma HLS inline self
  ...
  int raw_bits = tmp2(a[start], data_width,
                      (head_align * data_width));
...
static void
memcpy_wide_bus_single(write, float, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c, float c_buf,
                                   size_t offset_byte) {
#pragma HLS inline self
  const size_t data_width = sizeof(float);
  const size_t bus_width = BUS_WIDTH;
...
static double
memcpy_wide_bus_single(read, double, LARGE_BUS) (MARS_WIDE_BUS_TYPE *a,
                                   size_t offset_byte) {
#pragma HLS inline self
  ...
  long long raw_bits = tmp2(a[start], data_width,
                            (head_align * data_width));
...
static void
memcpy_wide_bus_single(write, double, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c, double c_buf,
                                    size_t offset_byte) {
#pragma HLS inline self
  const size_t data_width = sizeof(double);
  const size_t bus_width = BUS_WIDTH;
...
static void memcpy_wide_bus(read, long, LARGE_BUS) (long *a_buf, MARS_WIDE_BUS_TYPE *a,
                                            size_t offset_byte,
                                            size_t size_byte,
...
static void memcpy_wide_bus(write, long, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c, long *c_buf,
                                             size_t offset_byte,
                                             size_t size_byte, 
                                             const size_t const_buf_size) {
#pragma HLS inline self
...
static void memcpy_wide_bus(read, long_long, LARGE_BUS) (long long* a_buf, MARS_WIDE_BUS_TYPE *a,
                                            size_t offset_byte,
                                            size_t size_byte,
                                            const size_t const_buf_size
                                            ) {
...
static void memcpy_wide_bus(write, long_long, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c, long long *c_buf,
                                             size_t offset_byte,
                                             size_t size_byte, 
                                             const size_t const_buf_size) {
#pragma HLS inline self
...
static void memcpy_wide_bus(read, int, LARGE_BUS) (int *a_buf, MARS_WIDE_BUS_TYPE *a,
                                            size_t offset_byte,
                                            size_t size_byte,
                                            const size_t const_buf_size
                                            ) {
...
static void memcpy_wide_bus(write, int, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c, int *c_buf,
                                             size_t offset_byte,
                                             size_t size_byte, 
                                             const size_t const_buf_size) {
#pragma HLS inline self
...
static void memcpy_wide_bus(read, short, LARGE_BUS) (short *a_buf, MARS_WIDE_BUS_TYPE *a,
                                            size_t offset_byte,
                                            size_t size_byte,
                                            const size_t const_buf_size) {
#pragma HLS inline self
...
static void memcpy_wide_bus(write,short, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c, short *c_buf,
                                             size_t offset_byte,
                                             size_t size_byte,
                                             const size_t const_buf_size) {
#pragma HLS inline self 
...
static void memcpy_wide_bus(read, char, LARGE_BUS) (char *a_buf, MARS_WIDE_BUS_TYPE *a,
                                            size_t offset_byte,
                                            size_t size_byte,
                                            const size_t const_buf_size) {
#pragma HLS inline self
...
static void memcpy_wide_bus(write,char, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c, char *c_buf,
                                             size_t offset_byte,
                                             size_t size_byte,
                                             const size_t const_buf_size) {
#pragma HLS inline self 
...
static void memcpy_wide_bus(read,float, LARGE_BUS) (float *a_buf,
...
static void memcpy_wide_bus(write,float, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c,
                                               float *c_buf,
                                               size_t offset_byte,
                                               size_t size_byte, const size_t const_buf_size) {
#pragma HLS inline self
...
static void memcpy_wide_bus(read,double,LARGE_BUS) (double *a_buf,
                                               MARS_WIDE_BUS_TYPE *a,
                                               size_t offset_byte,
                                               size_t size_byte, const size_t const_buf_size) {
#pragma HLS inline self
...
static void memcpy_wide_bus(write,double, LARGE_BUS) (MARS_WIDE_BUS_TYPE *c,
                                                double *c_buf,
                                                size_t offset_byte,
                                                size_t size_byte, const size_t const_buf_size) {
#pragma HLS inline self
