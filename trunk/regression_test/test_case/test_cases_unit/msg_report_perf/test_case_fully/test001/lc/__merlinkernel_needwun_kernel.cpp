...
static void needwun_core(char SEQA[128],char SEQB[128],char alignedA[128 + 128],char alignedB[128 + 128])
{
//int M[(ALEN+1)*(BLEN+1)];
...
void needwun_kernel_L_0_2_para_sub(char alignedB_buf_0[256],char alignedA_buf_0[256],char SEQB_buf_0[128],char SEQA_buf_0[128],int i_sub)
...
#pragma HLS unroll complete
    needwun_kernel_L_0_2_para_sub(alignedB_buf_0[i_sub],alignedA_buf_0[i_sub],SEQB_buf_0[i_sub],SEQA_buf_0[i_sub],i_sub);
  }
}
...
#pragma HLS UNROLL
        merlin_memcpy_0(SEQA_buf_0[((long long )i_sub) - 0LL],0,SEQA,i * 8192 + i_sub * 128,sizeof(char ) * ((unsigned long )128),128UL);
      }
    }
{
      for (int i_sub = 0; i_sub < 64; ++i_sub) {
...
#pragma HLS UNROLL
        merlin_memcpy_1(SEQB_buf_0[((long long )i_sub) - 0LL],0,SEQB,i * 8192 + i_sub * 128,sizeof(char ) * ((unsigned long )128),128UL);
      }
    }
...
#pragma HLS UNROLL
        merlin_memcpy_2(alignedB,i * 16384 + i_sub * 256,alignedB_buf_0[((long long )i_sub) - 0LL],0,sizeof(char ) * ((unsigned long )256),256UL);
      }
    }
{
      for (int i_sub = 0; i_sub < 64; ++i_sub) {
...
#pragma HLS UNROLL
        merlin_memcpy_3(alignedA,i * 16384 + i_sub * 256,alignedA_buf_0[((long long )i_sub) - 0LL],0,sizeof(char ) * ((unsigned long )256),256UL);
      }
    }
//alignedB + (ALEN+BLEN) * i, M + ((ALEN+1)*(BLEN+1)) * i, ptr + ((ALEN+1)*(BLEN+1)) * i);
  }
