...
void vec_add_kernel_2(int *a, int *b, int*c, int inc);
void vec_add_kernel_3(int *a, int *b, int*c, int inc);
int main() {   
    int i;
    int inc = 0;
    int a[VEC_SIZE]; 
...
#pragma ACCEL task parallel name="vec_add_0"
//#pragma ACCEL task name="vec_add"
{
    ...
    vec_add_kernel_4(a, b, c, inc);
#pragma ACCEL task name="vec_add_1"
//#pragma ACCEL task name="vec_add"
{
    vec_add_kernel_2(a, b, c, inc);
    vec_add_kernel_3(a, b, c, inc);
}
    int cnt = 0;
    for(i = 0; i < VEC_SIZE; i++) {
        if(c[i] != a[i] + b[i]) {
