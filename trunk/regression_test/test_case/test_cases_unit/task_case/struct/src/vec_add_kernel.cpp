...
void vec_add_kernel1(struct st *a, int *b, int*c, int inc);
void vec_add_kernel2(struct st *d, int *b, int*c, int inc);

#pragma ACCEL task parallel
void vec_add_top(struct st *t_a, int *t_b, struct st *t_d, int*t_c1, int *t_c2, int t_inc)
{
    vec_add_kernel1(t_a, t_b, t_c1, t_inc);
    vec_add_kernel2(t_d, t_b, t_c2, t_inc);
}
