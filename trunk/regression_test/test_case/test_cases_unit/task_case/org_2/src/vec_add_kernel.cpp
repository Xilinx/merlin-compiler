...
void vec_add_kernel2(int *d, int *e, int*f, int inc)
{
    ...
    #pragma ACCEL interface variable=e depth=10000
    #pragma ACCEL interface variable=f depth=10000
    int j, j0, jj;
    for (j0 = 0 ;j0 < VEC_SIZE2/TILE; j0++) { 
        #pragma ACCEL parallel
        for (jj = 0; jj < TILE; jj++) {
            j = j0 * TILE + jj;
            f[j] = d[j]+e[j]+inc+10000;
        }
    }
}
