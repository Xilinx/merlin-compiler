...
// Standardize from: for(_s_i_0 =((unsigned char )4);_s_i_0 <= 15;_s_i_0 += 4) {...}
  for (_s_i_0 = 0; _s_i_0 <= 2; _s_i_0++) {
    int _in_s_i_0 = (int )(((long long )4) + 4LL * ((long long )_s_i_0));
...
// Standardize from: for(_s_i =((unsigned char )20);_s_i <= 31;_s_i += 4) {...}
  for (_s_i = 0; _s_i <= 2; _s_i++) {
    int _in_s_i = (int )(((long long )20) + 4LL * ((long long )_s_i));
...
#pragma ACCEL interface variable=data bus_bitwidth=8 max_depth=268435456 depth=268435456
  for (int i1 = 0; i1 < 16 * 1024 * 1024 / (1024 * 4); i1++)
// Original: #pragma ACCEL parallel factor=auto{options: CGPAR1=[1,2,4,8,16]; default:1}
// Original: #pragma ACCEL pipeline auto{options: CGPIP1=["off", ""]; order: 0 if v == "off" else 1; default:"off"}
{
...
// Original: #pragma ACCEL parallel factor=auto{options: CGPAR2=[x for x in [1,2,3,4,5,6,7,8,16]]; order: 0 if v & (v-1) == 0 else 1; default:1}
// Original: #pragma ACCEL pipeline auto{options: CGPIP2=["off", "", "flatten"]; order: 0 if v== "flatten" else 1; default:"off"}
