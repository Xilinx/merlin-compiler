...
#pragma ACCEL wrapper variable=c port=c depth=10000 max_depth=10000 data_type=int io=RW copy=false
...
#pragma ACCEL debug printf("[Merlin Info] Start %s data for %s, data size = %s...\n", "copy in", "c", "10000 * sizeof(int )");
      
#pragma ACCEL debug fflush(stdout);
      if (c != 0) {
        memcpy((void *)(__m_c + 0),(const void *)c,10000 * sizeof(int ));
      }
       else {
