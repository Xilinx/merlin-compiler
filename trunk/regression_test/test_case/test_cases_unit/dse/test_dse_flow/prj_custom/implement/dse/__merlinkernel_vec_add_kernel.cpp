...
#pragma ACCEL interface variable=a max_depth=10000 depth=10000
  int i, j;
  for (i = 0; i < 10; i++) {
#pragma ACCEL pipeline auto{options: I=["off", ""]; default:"off"}
      for (j = 0; j < 10000; j++) {
#pragma ACCEL parallel factor=auto{options: P=[x for x in range(10) + [16,32,64,128]]; \
                                   default:1} auto{options: L=["", "priority=logic"]; default:""}
        c[j] = a[j] + b[j] + inc;
      }
  }
