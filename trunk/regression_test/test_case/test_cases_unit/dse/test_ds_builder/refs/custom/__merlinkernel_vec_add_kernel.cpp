...
#pragma ACCEL interface variable=c max_depth=10000 depth=10000 bus_bitwidth=auto{ options: B=[32,64,128,256,512]; default: 32}
...
#pragma ACCEL pipeline AUTO{OPTIONS: I=["off", "", "flatten"]; DEFAULT: "off"; ORDER: 0 if v!="flatten" else 1}
    for (j = 0; j < 512; j++) {
      
#pragma ACCEL parallel factor=auto{options:P0=[2**x for x in range(0, 10) if x==0 or I != "flatten"]; default: 1} priority=logic
      foo(a,b,c);
    }
    foo(a,b,c);
    for (q = 0; q < 35; q++) {
...
#pragma ACCEL tiling factor=auto{options: T0=[2**x for x in range(0, 11) if T1 == 1]; default: 1; order: 0 if v <= 256 else 1 if v <= 512 else 2}
    for (j = 0; j < 512; j++) {
