...
void compute_normal_vector_v_kernel(bus_data_type data1[(240030 + 1 - 1) / 1],
//        int indices_t[INDEX_LENGTH], 
//        double origin_t[SAFE_LENGTH *NORMAL_LENGTH], 
double normal[(80010 + 18 - 1) / 18 * 3])
{
  double data1_1_2_buf[52];
  double data1_1_1_buf[52];
  double data1_1_0_buf[52];
#ifdef ADD_IF
...
///#pragma HLS INTERFACE m_axi port=origin offset=slave bundle=gmem3
  ...
  int start;
  for (start = 0; start < 80010; start += 18) {
    int n = 9;
    ;
    double cov[9];
...
//#pragma HLS array partition variable=accu1 complete dim=1
    LOOP_SLOW:
    memcpy(&data1_1_0_buf[0],&data1[3 * start],sizeof(double ) * 52);
    memcpy(&data1_1_1_buf[0],&data1[3 * start + 1],sizeof(double ) * 52);
    memcpy(&data1_1_2_buf[0],&data1[3 * start + 2],sizeof(double ) * 52);
    for (i = 0; i < 18; ++i) {
