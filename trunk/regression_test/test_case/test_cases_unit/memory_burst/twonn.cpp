...
#pragma ACCEL kernel
void pipeline(int n_samples,  float *global_data, 
...
//#pragma HLS UNROLL
#pragma ACCEL parallel complete
                    neuron1[j][k]+=data[j][LABEL_SIZE+p]*weights1[k][p+1];
                }
            }
        }
		/*
...
//#pragma HLS UNROLL
#pragma ACCEL parallel complete
                weights2[k][p] -= weights2[k][p] * two_lambda_learning_rate;
            }
        }
        for(j = 0; j < CHUNK_SIZE; j++)
        {
...
//#pragma HLS UNROLL
#pragma ACCEL parallel complete
                        weights1[k][p] -= error1[j][k] * (p==0 ? 1.f : data[j][LABEL_SIZE+p-1]);
                    }
                }
        }
#if DUMP
...
int main() {
        int n_samples;  float *global_data; 
        float weights1[HIDDEN_SIZE][FEATURE_SIZE+1]; 
        float bias2[LABEL_SIZE]; 
        float weights2[LABEL_SIZE][HIDDEN_SIZE];

pipeline( n_samples,   global_data, 
         weights1, 
         bias2, 
         weights2);
return 0;
