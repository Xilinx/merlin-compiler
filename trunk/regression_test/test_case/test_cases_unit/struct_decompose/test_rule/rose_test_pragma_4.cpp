...
void sub(char (*a_c)[12][10])
{
  
#pragma HLS array_partition variable=a_c dim=2
  
#pragma HLS array_partition variable=a_c dim=3
}
#pragma ACCEL kernel

void top(int a_a[10],char a_b[10],char a_c[10][12][10],char *b_b,int *c)
{
  
#pragma ACCEL INTERFACE DEPTH=100 VARIABLE=b_b max_depth=100
  
#pragma ACCEL INTERFACE DEPTH=100,12,10 VARIABLE=a_c max_depth=10,12,10
...
#pragma ACCEL FALSE_DEPENDENCE variable=a_c
    a_b[i] = ((char )(a_a[i] + ((int )b_b[i]) + ((int )tmp_c[i % 12][1]) + ((int )g_c[i % 12][2])));
  }
  return ;
}
