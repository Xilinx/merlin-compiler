...
void top(int *a_a,char *a_b,int *a_l_c_a,int *a_l_c_b,int *a_c,int *a_d)
{
  
#pragma ACCEL INTERFACE DEPTH=100 VARIABLE=a_d max_depth=100
...
#pragma ACCEL INTERFACE DEPTH=100 VARIABLE=a_a max_depth=100
  ...
  for (i = 0; i < 100; ++i) {
    ...
    a_b[i] = ((char )(((int )a_b[i]) + a_l_c_b[i]));
    a_c[i] = i + 1;
    a_d[i] = i * i;
  }
  return ;
...
#pragma ACCEL wrapper VARIABLE=a->l_c->b port=a_l_c_b data_type=int
  
#pragma ACCEL wrapper VARIABLE=a->c port=a_c data_type=int
  
#pragma ACCEL wrapper VARIABLE=a->d port=a_d data_type=int
...
#pragma ACCEL wrapper VARIABLE=a port=a data_type="struct st_parent"
  ...
  top(a_a,a_b,a_l_c_a,a_l_c_b,a_c,a_d);
  return 0;
}
