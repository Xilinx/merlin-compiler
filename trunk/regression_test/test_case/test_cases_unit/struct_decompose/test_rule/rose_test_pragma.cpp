...
#pragma HLS array_partition variable=a_c
}
#pragma ACCEL kernel

void top(int *a_a,char *a_b,char (*a_c)[12],char *b_b,int *c)
{
  
#pragma ACCEL INTERFACE DEPTH=100 VARIABLE=b_b max_depth=100
  
#pragma ACCEL INTERFACE DEPTH=100,12 VARIABLE=a_c max_depth=100,12
...
#pragma HLS array_partition variable=tmp_c dim=1
  
#pragma HLS array_partition variable=tmp_c dim=1
  
#pragma HLS array_partition variable=g_c dim = 1
  for (int i = 0; i < 12; ++i) {
    tmp_c[i] = ((char )c[i]);
  }
  sub(a_c);
