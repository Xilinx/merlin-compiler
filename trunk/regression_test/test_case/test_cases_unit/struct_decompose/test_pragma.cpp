...
#pragma HLS array_partition variable=a->c
}

#pragma ACCEL kernel
void top(struct st *a, struct st *b, int *c) {
#pragma ACCEL interface variable=a depth=100
#pragma ACCEL interface variable=b depth=100
  struct st tmp;
#pragma HLS array_partition variable=tmp->c dim=1
#pragma HLS array_partition variable=tmp.c dim=1
#pragma HLS array_partition variable=g.c  dim = 1
  for (int i = 0; i < 12; ++ i) {
    tmp.c[i] = c[i];
  }
  sub(a);
