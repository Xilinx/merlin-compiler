...
void adder(int *a,__global int * b,int inc)
{
  int j;
  for (j = 0; j < 10000; j++) {
    b[j] = a[j] + inc;
...
__kernel void vec_add_kernel(__global volatile int * restrict a,__global int * restrict b,__global int * restrict c,int inc)
{
  int b_buf_0[10000];
  long _memcpy_i_1;
