...
const int adder_2(__constant const int * a,int *b)
{
  int j;
  for (j = 0; j < 10000; j++) {
    b[j] = 2 * a[j];
...
int adder_3(__constant const int * a,int *b)
{
  int j;
  for (j = 0; j < 10000; j++) {
    b[j] = 2 * a[j];
...
int adder_4(__constant const int * a,int *b)
{
  int j;
  for (j = 0; j < 10000; j++) {
    b[j] = 2 * a[j];
...
int adder_5(__constant const int * a,int *b)
{
  int j;
  for (j = 0; j < 10000; j++) {
    b[j] = 2 * a[j];
...
int adder_6(__constant const long * a,int *b)
{
  int j;
  for (j = 0; j < 10000; j++) {
    b[j] = (2 * a[j]);
...
__kernel void vec_add_kernel(__constant const int * a,__global int * restrict b,__global int * restrict c,int inc)
{
  ...
  const int dd = 1;
  y = x;
  adder_2((__constant const int * )(a + 1),cc);
  adder_3((__constant const int * )(a - 1),cc);
  adder_4((__constant const int * )(&a[0]),cc);
  adder_5((__constant const int * )( *a),cc);
  adder_6(((__constant const long * )a) + 1,cc);
  int b_buf_0[10000];
  long _memcpy_i_1;
