...
void aes_mixColumns(unsigned char *buf)
{
  ...
  for (i = 0; i < 16; i += 4) {
    ...
    d = buf[i + 3];
    e = (a ^ b ^ c ^ d);
    buf[i] ^= e ^ (rj_xtime(((unsigned char )(((int )a) ^ ((int )b)))));
    buf[i + 1] ^= e ^ (rj_xtime(((unsigned char )(((int )b) ^ ((int )c)))));
    buf[i + 2] ^= e ^ (rj_xtime(((unsigned char )(((int )c) ^ ((int )d)))));
    buf[i + 3] ^= e ^ (rj_xtime(((unsigned char )(((int )d) ^ ((int )a)))));
  }
/* aes_mixColumns */
}
/* -------------------------------------------------------------------------- */
...
#pragma ACCEL interface variable=ctx depth=1 burst_off
  
#pragma ACCEL interface variable=k depth=32
  
#pragma ACCEL interface variable=buf depth=16
  ecb1:
