...
static void read(mywide_t (*inbuf)[32][512],hls::stream< mywide_t  > inStream[32])
{
  __merlin_access_range(inStream,0,31UL);
  __merlin_access_range(inbuf,0,131071UL);
//16
...
static void pageToByte(hls::stream< mywide_t  > &win,hls::stream< unsigned char  > &out)
{
  for (int i = 0; i < 2 * 1024 * 1024 / 16 / (8192 / 16) / 32; ++i) {
...
static void getHeaderInt2(hls::stream< unsigned char  > &stream,hls::stream< short  > nTupsStrm[6],hls::stream< unsigned char  > &outStream)
{
  __merlin_access_range(nTupsStrm,0,5UL);
  unsigned char ch1;
  unsigned char ch2;
  ch1 = stream . read();
  ch2 = stream . read();
...
static unsigned char p_ZN9TupParser7getByteER6streamIhE_1(struct TupParser *this_,hls::stream< unsigned char  > &stream)
{
  ++this_ -> pgOff;
  return stream . read();
}

static unsigned char p_ZN9TupParser7getByteER6streamIhE_2(struct TupParser *this_,hls::stream< unsigned char  > &stream)
{
  ++this_ -> pgOff;
  return stream . read();
}

static unsigned char p_ZN9TupParser7getByteER6streamIhE_3(struct TupParser *this_,hls::stream< unsigned char  > &stream)
{
  ++this_ -> pgOff;
  return stream . read();
}

static unsigned char p_ZN9TupParser7getByteER6streamIhE_4(struct TupParser *this_,hls::stream< unsigned char  > &stream)
{
  ++this_ -> pgOff;
  return stream . read();
}

static unsigned char p_ZN9TupParser7getByteER6streamIhE(struct TupParser *this_,hls::stream< unsigned char  > &stream)
{
  ++this_ -> pgOff;
  return stream . read();
}
...
static unsigned int p_ZN9TupParser11getFieldLenEjR6streamIhE(struct TupParser *this_,unsigned int fPtr,hls::stream< unsigned char  > &stream)
{
  unsigned char ch;
...
static bool p_ZN9TupParser8getFieldEPhjR6streamIhE(struct TupParser *this_,unsigned char fData[16],unsigned int fLen,hls::stream< unsigned char  > &stream)
{
  __merlin_access_range(fData,0,15UL);
...
static bool p_ZN9TupParser9skipBytesEiR6streamIhE(struct TupParser *this_,int fLen,hls::stream< unsigned char  > &stream)
{
  this_ -> off += fLen;
...
static void p_ZN9TupParser13skipToPageEndER6streamIhE(struct TupParser *this_,hls::stream< unsigned char  > &stream)
{
...
static void q6_filter(hls::stream< short  > &nTupsStrm,hls::stream< unsigned char  > &byteStream,hls::stream< fdata_t  > &qtyFieldStrm,hls::stream< fdata_t  > &extFieldStrm,hls::stream< fdata_t  > &dscFieldStrm,hls::stream< fdata_t  > &shpFieldStrm,hls::stream< bool  > &filterDone)
{
...
static void toLong(hls::stream< short  > &nTupsStrm,hls::stream< fdata_t  > &fieldStrm,hls::stream< long  > &longStrm)
{
  short nTups = nTupsStrm . read();
...
static void toDate(hls::stream< short  > &nTupsStrm,hls::stream< fdata_t  > &fieldStrm,hls::stream< int  > &dateStrm)
{
  short nTups = nTupsStrm . read();
  for (short i = (short )0; ((int )i) < ((int )nTups); ++i) {
...
static long agg(hls::stream< short  > &nTupsStrm,hls::stream< long  > &qtyStrm,hls::stream< long  > &extStrm,hls::stream< long  > &dscStrm,hls::stream< int  > &shpStrm,hls::stream< bool  > &filterDone)
{
  short nTups = nTupsStrm . read();
  long sum = (long )0;
  for (short i = (short )0; ((int )i) < ((int )nTups); ++i) {
...
static void q6_paged(hls::stream< unsigned char  > &byteStream,long *res)
{
...
static long q6(hls::stream< unsigned char  > &bStream)
{
...
{
  __merlin_access_range(outbuf,0,0UL);
  __merlin_access_range(inbuf,0,131071UL);
