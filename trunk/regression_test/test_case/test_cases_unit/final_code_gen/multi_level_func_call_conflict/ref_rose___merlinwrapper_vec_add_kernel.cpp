...
#pragma ACCEL wrapper variable = b port = b depth = 10000 max_depth = 10000 data_type = int io = WO copy = false
  }
...
#pragma ACCEL debug printf( "[Merlin Info] Start %s data for %s, data size = %s...\n", "copy out", "b", "10000 * sizeof(int )");
    
#pragma ACCEL debug fflush(stdout);
    if (b != ((int *)0)) {
      __CMOST_COMMENT__ __vec_add_kernel__b_buffer;
      opencl_read_buffer(__vec_add_kernel__b_buffer,commands[0],0 * sizeof(int ),(int *)b,((unsigned long )10000) * sizeof(int ));
    }
     else {
...
// __MERLIN_EVENT_READ__ __event___vec_add_kernel__b_buffer
      ...
      opencl_read_buffer_nb(__vec_add_kernel__b_buffer,commands[0],0 * sizeof(int ),(int *)b,((unsigned long )10000) * sizeof(int ),&__event___vec_add_kernel__b_buffer);
    }
     else {
      printf("Error! Detected null pointer 'b' for external memory.\n");
      exit(1);
