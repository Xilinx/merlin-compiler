...
static unsigned int Hash_2(__constant const char * p,int shift)
{
  return HashBytes( *((__constant const unsigned int * )p),shift);
//return HashBytes(UNALIGNED_LOAD32(p), shift);
}

static unsigned int Hash(__constant const char * p,int shift)
{
  return HashBytes( *((__constant const unsigned int * )p),shift);
//return HashBytes(UNALIGNED_LOAD32(p), shift);
}

static char *EmitLiteral_1(char *op,__constant const char * literal,int len,char allow_fast_path)
{
...
static char *EmitLiteral(char *op,__constant const char * literal,int len,char allow_fast_path)
{
...
static void FindMatchLength(unsigned long *ret_var_first,char *ret_var_second,__constant const char * s1,__constant const char * s2,__constant const char * s2_limit)
{
  int matched = 0;
  while(s2 <= s2_limit - 4 &&  *((__constant const unsigned int * )s2) ==  *((__constant const unsigned int * )(s2 + matched))){
//         UNALIGNED_LOAD32(s2) == UNALIGNED_LOAD32(s1 + matched)) {
    s2 += 4;
    matched += 4;
  }
  if (IsLittleEndian() && s2 <= s2_limit - 4) {
    unsigned int x =  *((__constant const unsigned int * )s2) ^  *((__constant const unsigned int * )(s1 + matched));
...
static __constant const char * GetEightBytesAt(__constant const char * ptr)
{
  return ptr;
}

static unsigned int GetUint32AtOffset_1(__constant const char * v,int offset)
{
  int offset_copy = 1;
  return  *((__constant const unsigned int * )(v + 1));
//return UNALIGNED_LOAD32(v + offset);
}

static unsigned int GetUint32AtOffset_2(__constant const char * v,int offset)
{
  int offset_copy = 1;
  return  *((__constant const unsigned int * )(v + 1));
//return UNALIGNED_LOAD32(v + offset);
...
static unsigned int GetUint32AtOffset_3(__constant const char * v,int offset)
{
  int offset_copy = 2;
  return  *((__constant const unsigned int * )(v + 2));
//return UNALIGNED_LOAD32(v + offset);
}

static unsigned int GetUint32AtOffset(__constant const char * v,int offset)
{
  int offset_copy = 0;
  return  *((__constant const unsigned int * )(v + 0));
//return UNALIGNED_LOAD32(v + offset);
}
static void __merlin_dummy_kBlockSize();
static void __merlin_dummy_kuint32max();
...
__kernel void CompressFragment_kernel(__constant const char * merlin_input,unsigned long input_size,__global char * restrict op,__global unsigned short * restrict table,const int table_size,__global int * restrict merlin_return_value)
{
...
// number of bytes to move ahead for each iteration.
      ...
      __constant const char * next_ip = ip;
      __constant const char * candidate;
      do {
        ip = next_ip;
      ...
      }while ( *((__constant const unsigned int * )ip) !=  *((__constant const unsigned int * )candidate));
...
// this loop via goto if we get close to exhausting the input.
      __constant const char * input_bytes;
...
// we also update table[Hash(ip - 1, shift)] and table[Hash(ip, shift)].
        ...
        candidate_bytes =  *((__constant const unsigned int * )candidate);
