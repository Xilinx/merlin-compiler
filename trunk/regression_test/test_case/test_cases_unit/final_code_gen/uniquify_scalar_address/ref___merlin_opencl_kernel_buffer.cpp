...
opencl_kernel __top2_kernel;
cl_event __event_top2;
opencl_mem __top2__a_buffer;
cl_event __event___top2__a_buffer;
int __merlin_init_top(){
	...
	#pragma ACCEL debug fflush(stdout);
	opencl_create_buffer(&__top__a_buffer, (int64_t)4, 2, NULL);
...
int __merlin_init_top2(){
	#pragma ACCEL debug printf("[Merlin Info] Start create kernel for %s....\n", "top2");
	#pragma ACCEL debug fflush(stdout);
	opencl_create_kernel(&__top2_kernel, (char *)"top2");
	#pragma ACCEL debug printf("[Merlin Info] Successful create kernel for %s....\n", "top2");
	#pragma ACCEL debug fflush(stdout);
	#pragma ACCEL debug printf("[Merlin Info] Start create buffer for %s....\n", "a");
	#pragma ACCEL debug fflush(stdout);
	opencl_create_buffer(&__top2__a_buffer, (int64_t)4, 2, NULL);
	#pragma ACCEL debug printf("[Merlin Info] Successful create buffer for %s....\n", "a");
	#pragma ACCEL debug fflush(stdout);
	return 0;
}
...
int __merlin_release_top2(){
	if(__top2_kernel) {
		opencl_release_kernel(__top2_kernel);
	}
	if(__event_top2) {
		opencl_release_event(__event_top2);
	}
	if(__top2__a_buffer) {
		opencl_release_mem_object(__top2__a_buffer);
	}
	return 0;
}
...
int __merlin_wait_kernel_top2(){
	opencl_wait_event(__event_top2);
	return 0;
}
