    ...
    if (head_align == 0 && aligned) {
      if (aligned == 1) {
        for (j = 0; j < num_elements; ++j) {
          if (i == end - start - 1 && j > tail_align)
            continue;
            int raw_bits = tmp2(((j + 1) * data_width * 8 - 1), (j * data_width * 8));
    ...
    if (head_align == 0 && aligned) {
      if (aligned == 1) {
        for (j = 0; j < num_elements; ++j) {
          if (i == end - start - 1 && j > tail_align)
            continue;
            long raw_bits = tmp2(((j + 1) * data_width * 8 - 1), (j * data_width * 8));
        ...
        long j0, j1;
        long bound = num_elements / SIZE_3;
        for (j0 = 0; j0 < bound; ++j0) {
          for (j1 = 0; j1 < SIZE_3; ++j1) {
            j = j0 * SIZE_3 + j1;
            if (i == end - start - 1 && j > tail_align)
        ...
        long j0, j1, j2;
        long bound = num_elements / T_SIZE_3;
        for (j0 = 0; j0 < bound; ++j0) {
          for (j1 = 0; j1 < SIZE_2; ++j1) {
            for (j2 = 0; j2 < SIZE_3; ++j2) {
              j = j0 * T_SIZE_3 + j1 * SIZE_3 + j2;
        ...
        long j0, j1, j2, j3;
        long bound = num_elements / T_SIZE;
        for (j0 = 0; j0 < bound; ++j0) {
          for (j1 = 0; j1 < SIZE_1; ++j1) {
            for (j2 = 0; j2 < SIZE_2; ++j2) {
              for (j3 = 0; j3 < SIZE_3; ++j3) {
