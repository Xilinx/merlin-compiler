...
int _local_r(int a,int _global_r)
{
  return a + _global_r;
}

int read_only_r(int a,int b)
{
  int write_only_r = 1;
  write_only_r += _local_r(1,2);
  return a + b + write_only_r;
}

__kernel void vec_add_kernel(__global int * restrict a,__global int * restrict b,__global int * restrict c,int global_r)
{
  int local_r = 1;
  int b_buf_0[10000];
  long _memcpy_i_1;
...
#pragma ACCEL interface variable=c max_depth=10000 depth=10000
  int j;
  for (j = 0; j < 10000; j++) {
    c_buf_0[j] = a_buf_0[j] + b_buf_0[j] + global_r + read_only_r(1,2) + local_r;
  }
  read_only_r(1,2);
  long _memcpy_i_2;
