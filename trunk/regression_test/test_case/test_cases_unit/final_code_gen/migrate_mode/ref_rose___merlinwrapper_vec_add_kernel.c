...
}
{
	...
	#pragma ACCEL debug clGetEventProfilingInfo(__event_vec_add_kernel, CL_PROFILING_COMMAND_END, sizeof(vec_add_kernel_end), &vec_add_kernel_end, NULL);
...
{
  int i;	#pragma ACCEL debug printf("[Merlin Info] Start create buffer for %s....\n", "b");
	#pragma ACCEL debug fflush(stdout);
	opencl_create_buffer(&__vec_add_kernel__b_buffer, (int64_t)4*10000, 2);
	#pragma ACCEL debug printf("[Merlin Info] Successful create buffer for %s....\n", "b");
	#pragma ACCEL debug fflush(stdout);
push //b
cl_event cl_event_vec_add_kernel_migrate_i
clEnqueueMigrateMemObjects(comands[0],1,v_cl_mem_vec_add_kernel_i,0,0,NULL,cl_event_vec_add_kernel_migrate_i);
{
	#pragma ACCEL debug printf("[Merlin Info] Start set kernel %s argument....\n", "vec_add_kernel");
	#pragma ACCEL debug fflush(stdout);
	opencl_set_kernel_arg(__vec_add_kernel_kernel, 0, sizeof(opencl_mem), &__vec_add_kernel__a_buffer);
