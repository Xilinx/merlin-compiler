...
//int n_samples = 60000;
  int n_samples = 64;
  float *global_weights_org;
  float *global_weights;
  float *global_weights_2;
  cmost_malloc_1d((&global_weights_org),"0",4,16 * (784 + 1));
  cmost_malloc_1d((&global_weights),"0",4,16 * 784);
  cmost_malloc_1d((&global_weights_2),"0",4,16 * 1);
//cmost_malloc_1d((&global_data),"0",4,60000 * (16 + 784));
  float *global_gradient;
  float *global_gradient_2;
  float *global_gradient_org;
  cmost_malloc_1d((&global_gradient),"0",4,16 * 784);
  cmost_malloc_1d((&global_gradient_2),"0",4,16 * 1);
...
//float * global_gradient_gold; cmost_malloc_1d( &global_gradient_gold, "gradient_gold.dat"  , 4, WEIGHTS_SIZE);
  ...
  float *global_data2;
  float *global_data;
  cmost_malloc_1d((&global_data),"global.dat",4,60000 * (16 + 784));
  ...
  for (p = 0; p < 16; p++) {
    for (n = 0; n < 784; n++) {
      global_weights[p * 784 + n] = global_weights_org[p * 785 + n];
    }
    global_weights_2[p] = global_weights_org[p * 785 + 784];
  }
  ...
  logistic_regression_kernel(n_samples,global_weights,global_weights_2,global_data1,global_data2,global_gradient,global_gradient_2);
  int i;
  int err = 0;
  double epsilon = 1.0e-16;
  for (p = 0; p < 784; p++) {
    for (n = 0; n < 16; n++) {
      global_gradient_org[n * 785 + p] = global_gradient[p * 16 + n];
    }
  }
  for (n = 0; n < 16; n++) {
    global_gradient_org[n * 785 + 784] = global_gradient_2[n];
  }
  printf("data[%d] = %f\n",p,global_gradient_2[0]);
  for (p = 0; p < 10; p++) {
    printf("data[%d] = %f\n",p,global_gradient_org[p]);
  }
...
//cmost_write_file(global_gradient, "gradient.dat", 4*WEIGHTS_SIZE); // added
  cmost_dump(global_gradient_org,"gradient_out.dat");
  cmost_free_1d(global_weights_org);
  cmost_free_1d(global_data);
  cmost_free_1d(global_gradient_org);
  cmost_free_1d(global_weights);
  cmost_free_1d(global_data1);
  cmost_free_1d(global_gradient);
  cmost_free_1d(global_weights_2);
  cmost_free_1d(global_data2);
  cmost_free_1d(global_gradient_2);
