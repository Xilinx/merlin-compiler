  ...
  for (i = 0; i < n_samples / 32; i++) {
    ...
    for (_memcpy_i0_1 = 0; _memcpy_i0_1 < 32; ++_memcpy_i0_1) {
      for (_memcpy_i0_0 = 0; _memcpy_i0_0 < 784; ++_memcpy_i0_0) {
        data[_memcpy_i0_1][_memcpy_i0_0] = global_data[i * 32 * 784 + ((0 * 32 + _memcpy_i0_1) * 784 + _memcpy_i0_0)];
      }
    }
...
//result1[j][k] += weights[i+1][k] * data[j][i];
            result1_t__compute1[j__compute1][k__compute1] += global_weights[k__compute1 * 785 + (i__compute1 + 1)] * data[j__compute1][i__compute1];
            if (i__compute1 == 783) {
    ...
    }
{
      ...
      int j__compute3;
      int k__compute3;
      float gradient__compute3[784 + 1][16];
      ...
      for (j__compute3 = 0; j__compute3 < 32; j__compute3++) {
        for (i__compute3 = 0; i__compute3 < 784 + 1; i__compute3++) {
          for (k__compute3 = 0; k__compute3 < 16; k__compute3++) {
            ...
            gradient__compute3[i__compute3][k__compute3] += result2_t__compute3[j__compute3][k__compute3] * ((i__compute3 == 0?1.f : data[j__compute3][i__compute3 - 1]));
            if (k__compute3 == 16 - 1) {
