...
#pragma ACCEL PARALLEL COMPLETE
                  Cin_tmp_0_0[h][w][ii] = Cin_buf_0_0[h][w][ii];
                }
              }
              if (h >= 3 - 1 && w >= 3 - 1) {
                for (int ii = 0; ii < 4; ii++) {
...
#pragma ACCEL PARALLEL COMPLETE
                  if (p == 0 && q == 0 && j0 == 0) {
                    Cout_buf_tmp_0_0[ii] = bias_buf_0[i0][ii];
                  }
                   else {
...
#pragma ACCEL PARALLEL COMPLETE
                    Cout_buf_tmp_0_0[ii] += weight_buf_0_0[i0][j0][p][q][jj][ii] * Cin_tmp_0_0[h + p - 3 + 1][w + q - 3 + 1][jj];
...
// Cout[(i0 * TILE + ii) * IMROW * IMROW + h * IMROW + w] =
                  ...
                  Cout_buf_0_0_rn[- 2 + h][- 2 + w][ii] = Cout_buf_0_0_tmp;
                }
              }
            }
          }
