...
#pragma ACCEL interface variable=merlin_in depth=16,32,32
  
#pragma ACCEL interface variable=weight depth=64,3,3,32
  
#pragma ACCEL interface variable=merlin_out depth=16,32,64
      ...
      for (int c_outer = 0; c_outer < 1; ++c_outer) 
        for (int r_outer = 0; r_outer < 2; ++r_outer) 
          for (int i_outer = 0; i_outer < 1; ++i_outer) 
            for (int o_outer = 0; o_outer < 2; ++o_outer) {
              ...
              for (int i_feeder = 0; i_feeder < 4; ++i_feeder) 
                for (int q_feeder = 0; q_feeder < 3; ++q_feeder) 
                  for (int p_feeder = 0; p_feeder < 3; ++p_feeder) 
                    for (int r_feeder = 0; r_feeder < 8; ++r_feeder) 
                      for (int c_feeder = 0; c_feeder < 1; ++c_feeder) 
                        for (int o_feeder = 0; o_feeder < 7; ++o_feeder) {
                          
#pragma ACCEL parallel scope
                          for (int c_row_pe = 0; c_row_pe < 32; ++c_row_pe) 
                            for (int o_col_pe = 0; o_col_pe < 5; ++o_col_pe) 
                              for (int i_simd_pe = 0; i_simd_pe < 8; ++i_simd_pe) {
                                int o_sys = o_outer * 35 + (o_col_pe * 7 + o_feeder);
                                int i_sys = i_outer * 32 + (i_simd_pe * 4 + i_feeder);
                                int r_sys = r_outer * 8 + r_feeder;
                                int c_sys = c_outer * 32 + (c_row_pe * 1 + c_feeder);
