...
int main(int argc, char **argv)
{
	...
	float *b;
	float *c;
	unsigned int num_elem_a = (BN_R * BN_C * BN_I) * (BS_R * R + K - 1) * (BS_C + K - 1) * (BS_I * I);
	unsigned int num_elem_b = (BN_I * BS_I * I) * (BN_O * BS_O * O) * K * K;
	unsigned int num_elem_c = (BN_R * BS_R * R) * (BN_C * BS_C) * (BN_O * BS_O * O);
