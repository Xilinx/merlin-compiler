        ...
        for (int k_feeder = 0; k_feeder < 5; ++k_feeder) {
          for (int i_row_pe = 0; i_row_pe < 4; ++i_row_pe) {
            for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
              int in_idx = ((i_outer * 4 + i_row_pe) * 10 + i_feeder) * 75 + ((k_outer * 5 + k_feeder) * 8 + k_simd_pe);
              int in_bw_idx = ((i_outer * 2 + k_outer) * 40 + (i_row_pe * 10 + i_feeder)) * 40 + (k_feeder * 8 + k_simd_pe);
              if ((i_outer * 4 + i_row_pe) * 10 + i_feeder < 43 && (k_outer * 5 + k_feeder) * 8 + k_simd_pe < 75) 
      ...
      for (int j_feeder = 0; j_feeder < 1; ++j_feeder) {
        for (int k_feeder = 0; k_feeder < 5; ++k_feeder) {
          for (int j_col_pe = 0; j_col_pe < 40; ++j_col_pe) {
            for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
              int w_idx = (j_outer * 40 + j_col_pe + j_feeder) * 75 + ((k_outer * 5 + k_feeder) * 8 + k_simd_pe);
              int w_bw_idx = ((j_outer * 2 + k_outer) * 40 + (j_col_pe + j_feeder)) * 40 + (k_feeder * 8 + k_simd_pe);
              if (j_outer * 40 + j_col_pe + j_feeder < 35 && (k_outer * 5 + k_feeder) * 8 + k_simd_pe < 75) 
    ...
    for (int i_row_pe = 0; i_row_pe < 4; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 10; ++i_feeder) {
        for (int j_outer = 0; j_outer < 1; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 40; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 1; ++j_feeder) {
              int out_idx = ((i_outer * 4 + i_row_pe) * 10 + i_feeder) * 35 + (j_outer * 40 + j_col_pe + j_feeder);
              int out_bw_idx = ((i_outer + j_outer) * 40 + (i_row_pe * 10 + i_feeder)) * 40 + (j_col_pe + j_feeder);
              if ((i_outer * 4 + i_row_pe) * 10 + i_feeder < 43 && j_outer * 40 + j_col_pe + j_feeder < 35) 
    ...
    for (int i_row_pe = 0; i_row_pe < 4; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 10; ++i_feeder) {
        for (int j_outer = 0; j_outer < 1; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 40; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 1; ++j_feeder) {
              int out_idx = ((i_outer + j_outer) * 40 + (i_row_pe * 10 + i_feeder)) * 40 + (j_col_pe + j_feeder);
              int out_bw_idx = ((i_outer + j_outer) * 40 + (i_row_pe * 10 + i_feeder)) * 40 + (j_feeder * 40 + j_col_pe);
