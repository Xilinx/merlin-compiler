...
#pragma ACCEL interface variable=A depth=1,1327
  
#pragma ACCEL interface variable=B depth=1,1327
  
#pragma ACCEL interface variable=C depth=1,1
              ...
              for (int i_row_pe = 0; i_row_pe < 10; ++i_row_pe) 
                for (int j_col_pe = 0; j_col_pe < 12; ++j_col_pe) 
                  for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
                    int j_sys = j_outer * 12 + (j_col_pe * 1 + j_feeder);
                    int i_sys = i_outer * 10 + (i_row_pe * 1 + i_feeder);
