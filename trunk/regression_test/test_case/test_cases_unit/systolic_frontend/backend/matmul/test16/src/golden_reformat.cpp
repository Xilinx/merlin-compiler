...
void block_wise_reformat_input(data_precision *input,data_precision *input_block_wise)
{
  for (int i_outer = 0; i_outer < 2; ++i_outer) {
    for (int k_outer = 0; k_outer < 4; ++k_outer) {
      for (int i_feeder = 0; i_feeder < 5; ++i_feeder) {
        for (int k_feeder = 0; k_feeder < 5; ++k_feeder) {
          for (int i_row_pe = 0; i_row_pe < 4; ++i_row_pe) {
            for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
              int in_idx = ((i_outer * 4 + i_row_pe) * 5 + i_feeder) * 137 + ((k_outer * 5 + k_feeder) * 8 + k_simd_pe);
              int in_bw_idx = ((i_outer * 4 + k_outer) * 20 + (i_row_pe * 5 + i_feeder)) * 40 + (k_feeder * 8 + k_simd_pe);
              if ((i_outer * 4 + i_row_pe) * 5 + i_feeder < 23 && (k_outer * 5 + k_feeder) * 8 + k_simd_pe < 137) 
      ...
      for (int j_feeder = 0; j_feeder < 1; ++j_feeder) {
        for (int k_feeder = 0; k_feeder < 5; ++k_feeder) {
          for (int j_col_pe = 0; j_col_pe < 40; ++j_col_pe) {
            for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
              int w_idx = (j_outer * 40 + j_col_pe + j_feeder) * 137 + ((k_outer * 5 + k_feeder) * 8 + k_simd_pe);
              int w_bw_idx = ((j_outer * 4 + k_outer) * 40 + (j_col_pe + j_feeder)) * 40 + (k_feeder * 8 + k_simd_pe);
              if (j_outer * 40 + j_col_pe + j_feeder < 37 && (k_outer * 5 + k_feeder) * 8 + k_simd_pe < 137) 
    ...
    for (int i_row_pe = 0; i_row_pe < 4; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 5; ++i_feeder) {
        for (int j_outer = 0; j_outer < 1; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 40; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 1; ++j_feeder) {
              int out_idx = ((i_outer * 4 + i_row_pe) * 5 + i_feeder) * 37 + (j_outer * 40 + j_col_pe + j_feeder);
              int out_bw_idx = ((i_outer + j_outer) * 20 + (i_row_pe * 5 + i_feeder)) * 40 + (j_col_pe + j_feeder);
              if ((i_outer * 4 + i_row_pe) * 5 + i_feeder < 23 && j_outer * 40 + j_col_pe + j_feeder < 37) 
    ...
    for (int i_row_pe = 0; i_row_pe < 4; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 5; ++i_feeder) {
        for (int j_outer = 0; j_outer < 1; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 40; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 1; ++j_feeder) {
              int out_idx = ((i_outer + j_outer) * 20 + (i_row_pe * 5 + i_feeder)) * 40 + (j_col_pe + j_feeder);
              int out_bw_idx = ((i_outer + j_outer) * 20 + (i_row_pe * 5 + i_feeder)) * 40 + (j_feeder * 40 + j_col_pe);
