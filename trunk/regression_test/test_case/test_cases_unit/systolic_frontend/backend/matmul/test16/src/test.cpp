...
#pragma ACCEL interface variable=A depth=23,137
  
#pragma ACCEL interface variable=B depth=37,137
  
#pragma ACCEL interface variable=C depth=23,37
              ...
              for (int i_row_pe = 0; i_row_pe < 4; ++i_row_pe) 
                for (int j_col_pe = 0; j_col_pe < 40; ++j_col_pe) 
                  for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
                    ...
                    int i_sys = i_outer * 20 + (i_row_pe * 5 + i_feeder);
