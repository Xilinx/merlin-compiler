...
void block_wise_reformat_input(data_precision *input,data_precision *input_block_wise)
{
  for (int i_outer = 0; i_outer < 2; ++i_outer) {
    for (int k_outer = 0; k_outer < 2; ++k_outer) {
      for (int i_feeder = 0; i_feeder < 6; ++i_feeder) {
        for (int k_feeder = 0; k_feeder < 8; ++k_feeder) {
          for (int i_row_pe = 0; i_row_pe < 10; ++i_row_pe) {
            for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
              int in_idx = ((i_outer * 10 + i_row_pe) * 6 + i_feeder) * 128 + ((k_outer * 8 + k_feeder) * 8 + k_simd_pe);
              int in_bw_idx = ((i_outer * 2 + k_outer) * 60 + (i_row_pe * 6 + i_feeder)) * 64 + (k_feeder * 8 + k_simd_pe);
              if ((i_outer * 10 + i_row_pe) * 6 + i_feeder < 64 && (k_outer * 8 + k_feeder) * 8 + k_simd_pe < 128) 
    ...
    for (int i_row_pe = 0; i_row_pe < 10; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 6; ++i_feeder) {
        for (int j_outer = 0; j_outer < 1; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 16; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 4; ++j_feeder) {
              int out_idx = ((i_outer * 10 + i_row_pe) * 6 + i_feeder) * 64 + ((j_outer * 16 + j_col_pe) * 4 + j_feeder);
              int out_bw_idx = ((i_outer + j_outer) * 60 + (i_row_pe * 6 + i_feeder)) * 64 + (j_col_pe * 4 + j_feeder);
              if ((i_outer * 10 + i_row_pe) * 6 + i_feeder < 64 && (j_outer * 16 + j_col_pe) * 4 + j_feeder < 64) 
    ...
    for (int i_row_pe = 0; i_row_pe < 10; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 6; ++i_feeder) {
        for (int j_outer = 0; j_outer < 1; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 16; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 4; ++j_feeder) {
              int out_idx = ((i_outer + j_outer) * 60 + (i_row_pe * 6 + i_feeder)) * 64 + (j_col_pe * 4 + j_feeder);
              int out_bw_idx = ((i_outer + j_outer) * 60 + (i_row_pe * 6 + i_feeder)) * 64 + (j_feeder * 16 + j_col_pe);
