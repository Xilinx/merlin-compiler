...
{
#pragma ACCEL interface variable=A depth=16,327
#pragma ACCEL interface variable=B depth=16,327
              ...
              for (int i_row_pe = 0; i_row_pe < 20; ++i_row_pe) 
                for (int j_col_pe = 0; j_col_pe < 8; ++j_col_pe) 
                  for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
                    int j_sys = j_outer * 16 + (j_col_pe * 2 + j_feeder);
                    int i_sys = i_outer * 20 + (i_row_pe * 1 + i_feeder);
                    int k_sys = k_outer * 16 + (k_simd_pe * 2 + k_feeder);
