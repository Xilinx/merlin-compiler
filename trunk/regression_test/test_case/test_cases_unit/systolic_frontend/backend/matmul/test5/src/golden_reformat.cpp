...
void block_wise_reformat_input(data_precision *input,data_precision *input_block_wise)
{
  for (int i_outer = 0; i_outer < 2; ++i_outer) {
    for (int k_outer = 0; k_outer < 2; ++k_outer) {
      for (int i_feeder = 0; i_feeder < 12; ++i_feeder) {
        for (int k_feeder = 0; k_feeder < 21; ++k_feeder) {
          for (int i_row_pe = 0; i_row_pe < 11; ++i_row_pe) {
            for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
              int in_idx = ((i_outer * 11 + i_row_pe) * 12 + i_feeder) * 327 + ((k_outer * 21 + k_feeder) * 8 + k_simd_pe);
              int in_bw_idx = ((i_outer * 2 + k_outer) * 132 + (i_row_pe * 12 + i_feeder)) * 168 + (k_feeder * 8 + k_simd_pe);
              if ((i_outer * 11 + i_row_pe) * 12 + i_feeder < 256 && (k_outer * 21 + k_feeder) * 8 + k_simd_pe < 327) 
      ...
      for (int j_feeder = 0; j_feeder < 12; ++j_feeder) {
        for (int k_feeder = 0; k_feeder < 21; ++k_feeder) {
          for (int j_col_pe = 0; j_col_pe < 14; ++j_col_pe) {
            for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
              int w_idx = ((j_outer * 14 + j_col_pe) * 12 + j_feeder) * 327 + ((k_outer * 21 + k_feeder) * 8 + k_simd_pe);
              int w_bw_idx = ((j_outer * 2 + k_outer) * 168 + (j_col_pe * 12 + j_feeder)) * 168 + (k_feeder * 8 + k_simd_pe);
              if ((j_outer * 14 + j_col_pe) * 12 + j_feeder < 256 && (k_outer * 21 + k_feeder) * 8 + k_simd_pe < 327) 
    ...
    for (int i_row_pe = 0; i_row_pe < 11; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 12; ++i_feeder) {
        for (int j_outer = 0; j_outer < 2; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 14; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 12; ++j_feeder) {
              int out_idx = ((i_outer * 11 + i_row_pe) * 12 + i_feeder) * 256 + ((j_outer * 14 + j_col_pe) * 12 + j_feeder);
              int out_bw_idx = ((i_outer * 2 + j_outer) * 132 + (i_row_pe * 12 + i_feeder)) * 168 + (j_col_pe * 12 + j_feeder);
              if ((i_outer * 11 + i_row_pe) * 12 + i_feeder < 256 && (j_outer * 14 + j_col_pe) * 12 + j_feeder < 256) 
    ...
    for (int i_row_pe = 0; i_row_pe < 11; ++i_row_pe) {
      for (int i_feeder = 0; i_feeder < 12; ++i_feeder) {
        for (int j_outer = 0; j_outer < 2; ++j_outer) {
          for (int j_col_pe = 0; j_col_pe < 14; ++j_col_pe) {
            for (int j_feeder = 0; j_feeder < 12; ++j_feeder) {
              int out_idx = ((i_outer * 2 + j_outer) * 132 + (i_row_pe * 12 + i_feeder)) * 168 + (j_col_pe * 12 + j_feeder);
              int out_bw_idx = ((i_outer * 2 + j_outer) * 132 + (i_row_pe * 12 + i_feeder)) * 168 + (j_feeder * 14 + j_col_pe);
