...
#pragma ACCEL interface variable=A depth=256,327
  
#pragma ACCEL interface variable=B depth=256,327
              ...
              for (int i_row_pe = 0; i_row_pe < 11; ++i_row_pe) 
                for (int j_col_pe = 0; j_col_pe < 14; ++j_col_pe) 
                  for (int k_simd_pe = 0; k_simd_pe < 8; ++k_simd_pe) {
                    int j_sys = j_outer * 168 + (j_col_pe * 12 + j_feeder);
                    int i_sys = i_outer * 132 + (i_row_pe * 12 + i_feeder);
                    int k_sys = k_outer * 168 + (k_simd_pe * 21 + k_feeder);
