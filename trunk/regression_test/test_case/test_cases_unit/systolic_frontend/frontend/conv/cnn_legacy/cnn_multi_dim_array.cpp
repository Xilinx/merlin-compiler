...
void compute_gold(
	float out[IMG_OUT_NUM][IMG_ROW][IMG_COL], 
	const float in[IMG_IN_NUM][IMG_IN_ROW][IMG_IN_COL],
	const float weight[IMG_OUT_NUM][IMG_IN_NUM][KERNEL][KERNEL]
) {
...
#pragma ACCEL systolic auto=auto
	...
	for (int q = 0; q < KERNEL; q++) {
			out[o][r][c] += weight[o][i][p][q] * in[i][r + p][c + q];
	}
