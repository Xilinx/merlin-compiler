...
void vec_add_kernel_s1(int *b, int*c, st inc);

#pragma ACCEL task parallel
void vec_add_kernel(int * a, int * b, int *c, st inc)
{
    vec_add_kernel_s0(a);
...
#pragma ACCEL kernel
void vec_add_kernel_s1(int b[VEC_SIZE], int c[VEC_SIZE], st inc)
{

    for (int j = 0; j < VEC_SIZE; j++)
    {
        int tmp;
        merlin_stream_read(&ch_a_0,(&tmp));
        c[j] = tmp+b[j]+inc.val;
    }
}
