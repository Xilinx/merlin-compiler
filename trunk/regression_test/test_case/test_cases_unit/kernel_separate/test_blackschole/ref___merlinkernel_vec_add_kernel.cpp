...
static float CNDF_1(float InputX)
{
  int sign;
  float OutputX;
  float xInput;
...
// Compute NPrimeX term common to both four & six decimal accuracy calcs
  expValues = my_exp_1((float )(- 0.5f * InputX * InputX));
...
static float BlkSchlsEqEuroNoDiv_1(float sptprice,float strike,float rate,float volatility,float time,int otype,float timet)
{
  float OptionPrice;
...
//logValues = log( sptprice / strike );
  ...
  NofXd1 = CNDF_1(d1);
  NofXd2 = CNDF_1(d2);
  FutureValueX = strike * my_exp_1((float )(-rate * time));
  if (otype == 0) {
...
//#pragma ACCEL pipeline
    for (ii = 0; ii < BLOCK_SIZE; ii++) {
      ...
      price = BlkSchlsEqEuroNoDiv_1(curr_sptprice,curr_strike,curr_rate,curr_volatility,curr_otime,(int )curr_otype,(float )0);
