...
// kernel prototype
//
void vec_add_kernel(float *target,float *target_1,float *target_2,float *query,float *query_1,float *query_2,int *idx);
// inputs
...
// allocate storage for SW version
  ...
  query && target && tree_idx && tree_depth && tree_left_idx && tree_right_idx && tree_x && tree_y && tree_z && idx && sw_idx?((void )0) : __assert_fail("query && target && tree_idx && tree_depth && tree_left_idx && tree_right_idx && tree_x && tree_y && tree_z && idx && sw_idx","vec_add.cpp",351,__PRETTY_FUNCTION__);
...
// convert floating data into fixed data
  for (int i = 0; i != 18176 * 3 / 3; ++i) {
    target_x_hw[i] = target[i * 3 + 0];
    target_y_hw[i] = target[i * 3 + 1];
    target_z_hw[i] = target[i * 3 + 2];
  }
  for (int i = 0; i != 9600 * 3 / 3; ++i) {
    query_x_hw[i] = query[i * 3 + 0];
    query_y_hw[i] = query[i * 3 + 1];
    query_z_hw[i] = query[i * 3 + 2];
  }
  printf("Calling FLANN FPGA version now ...\n");
  
  vec_add_kernel(target_x_hw,target_y_hw,target_z_hw,query_x_hw,query_y_hw,query_z_hw,idx);
// inputs
// outputs
