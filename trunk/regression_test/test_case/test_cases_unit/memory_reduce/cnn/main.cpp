...
template < int OUTF, int HEI, int WID > int Verify ( float Cout [ OUTF ] [ HEI ] [ WID ] ) {
  ...
  for ( int i0 = 0; i0 < OUTF / 8; i0 ++ ) {
    for ( int j = 0; j < HEI; j ++ ) {
      for ( int k = 0; k < WID; k ++ ) {
        for ( int ii = 0; ii < 8; ii ++ ) {
          int i = i0 * 8 + ii;
          if ( rcmp ( * ( & ( Cout [ 0 ] [ 0 ] [ 0 ] ) + i0 * 224 * 224 * 8 + j * 224 * 8 +
                     k * 8 + ii ),
  ...
  for (int j0 = 0; j0 < 32 / 8; j0++) {
    for (int _memcpy_i0_1 = 0; _memcpy_i0_1 < 226; ++_memcpy_i0_1) {
      for (int _memcpy_i0_0 = 0; _memcpy_i0_0 < 226; ++_memcpy_i0_0) {
        for (int _memcpy_i0_2 = 0; _memcpy_i0_2 < 8; ++_memcpy_i0_2) {
          Cin_hw[j0 * 8 * 226 * 226 + _memcpy_i0_1 * 226 * 8 + _memcpy_i0_0 * 8 + _memcpy_i0_2] = Cin[j0 * 8 + _memcpy_i0_2][_memcpy_i0_1][_memcpy_i0_0];
        }
      }
    }
  }
      ...
      for (int __memcpy_i_1_0 = 0; __memcpy_i_1_0 < 3; ++__memcpy_i_1_0) {
        for (int __memcpy_i_1_1 = 0; __memcpy_i_1_1 < 3; ++__memcpy_i_1_1) {
          for (int jj = 0; jj < 8; jj++) {
            for (int ii = 0; ii < 8; ii++) {
              weight_hw[i0 * 32 * 8 * 3 * 3 + j0 * 3 * 3 * 8 * 8 + __memcpy_i_1_0 * 3 * 8 * 8 + __memcpy_i_1_1 * 8 * 8 + jj * 8 + ii] = weight[i0 * 8 + ii][j0 * 8 + jj][__memcpy_i_1_0][__memcpy_i_1_1];
            }
          }
        }
      }
