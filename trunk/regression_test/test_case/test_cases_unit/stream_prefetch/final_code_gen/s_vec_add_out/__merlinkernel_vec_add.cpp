...
merlin_stream ch_b_buf_0_1_p[4];
#pragma ACCEL STREAM_CHANNEL CHANNEL_DEPTH=1 NODE=msm_node_0 PORT=msm_port_a_buf_0_msm_node_0_0
merlin_stream ch_a_buf_0_0_p[4];
#include <string.h> 
#include "merlin_stream.h" 
static void __merlin_dummy_extern_int_merlin_include_G_();
static void __merlin_dummy_extern_int_merlin_include_G_();
#pragma ACCEL STREAM_CHANNEL CHANNEL_DEPTH=32 NODE=vec_add PORT=msm_port_c_vec_add_0
merlin_stream ch_c_0_p[4];
#include "cmost.h"
#include <math.h>
...
{
      int c_sp_tmp_0;
      merlin_stream_read(&ch_c_0_p[_i_sub],(&c_sp_tmp_0));
...
void msm_node_0(int a_buf_0[4][1],int b_buf_0[4][1],int s,int i_sub)
// Original: #pragma ACCEL parallel factor=4
// Original: #pragma ACCEL PARALLEL FACTOR=4
{
  int _i_sub;
  ...
  int c_sn_tmp_0;
  merlin_stream_read(&ch_a_buf_0_0_p[i_sub],(&a_buf_0_sn_tmp_0));
  int a0 = a_buf_0_sn_tmp_0;
  merlin_stream_read(&ch_b_buf_0_1_p[i_sub],(&b_buf_0_sn_tmp_0));
  ...
  c_sn_tmp_0 = c0;
  merlin_stream_write(&ch_c_0_p[i_sub],(void *)(&c_sn_tmp_0));
}
...
void msm_port_a_buf_0_msm_node_0_0(int a_buf_0[4][1],int i_sub)
{
  ...
  merlin_stream_write(&ch_a_buf_0_0_p[i_sub],(&a_buf_0_sp_tmp_0));
}
...
void msm_port_b_buf_0_msm_node_0_0(int b_buf_0[4][1],int i_sub)
{
  ...
  merlin_stream_write(&ch_b_buf_0_1_p[i_sub],(&b_buf_0_sp_tmp_0));
}
...
#pragma unroll
  for (_i_sub = 0; _i_sub <= 3; ++_i_sub) {
    merlin_stream_init(&ch_c_0_p[_i_sub],(unsigned long )1024,(unsigned long )4);
  }
  int i_sub;
...
#pragma unroll
    for (i_sub = 0; i_sub < 4; ++i_sub) {
      merlin_stream_init(&ch_a_buf_0_0_p[i_sub],4,4);
// Original pragma: ACCEL STREAM_PORT ACCESS_TYPE=read DIM_NUM=2 NODE=msm_node_0 VARIABLE=a_buf_0
      
#pragma ACCEL STREAM_PORT ACCESS_TYPE=read DIM_NUM=2 NODE=msm_node_0 PARALLEL_ITERATOR=1 VARIABLE=a_buf_0
      msm_port_a_buf_0_msm_node_0_0(a_buf_0,i_sub);
      merlin_stream_init(&ch_b_buf_0_1_p[i_sub],4,4);
...
#pragma ACCEL STREAM_NODE PARALLEL_FACTOR=4 PARALLEL_ITERATOR=3
      msm_node_0(a_buf_0,b_buf_0,s,i_sub);
      merlin_stream_reset(&ch_s_2_p[i_sub]);
      merlin_stream_reset(&ch_b_buf_0_1_p[i_sub]);
      merlin_stream_reset(&ch_a_buf_0_0_p[i_sub]);
    }
  }
...
#pragma unroll
  for (_i_sub_0 = 0; _i_sub_0 <= 3; ++_i_sub_0) {
    merlin_stream_reset(&ch_c_0_p[_i_sub_0]);
  }
}
