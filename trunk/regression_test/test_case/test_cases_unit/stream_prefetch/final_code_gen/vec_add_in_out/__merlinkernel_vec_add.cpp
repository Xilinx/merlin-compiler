...
// Original: #pragma ACCEL array_partition type=channel variable=ch_s_1 dim=1 factor=4
...
// Original: #pragma ACCEL array_partition type=channel variable=ch_a_1 dim=1 factor=4
...
{
      int a_sp_tmp_1;
      a_sp_tmp_1 = a_buf_0[i][_i_sub];
      merlin_stream_write(&ch_a_1_p[_i_sub],(&a_sp_tmp_1));
    }
  }
}
...
#pragma unroll
    for (_memcpy_i1_0 = 0; _memcpy_i1_0 < 4; ++_memcpy_i1_0) {
      c[0 + ((0 * 256 + _memcpy_i1_1) * 4 + _memcpy_i1_0)] = c_buf_0[_memcpy_i1_1][_memcpy_i1_0];
    }
  }
}

void msm_node_0(int b_buf_0[4][1],int s,int i_sub)
// Original: #pragma ACCEL parallel factor=4
// Original: #pragma ACCEL PARALLEL FACTOR=4
{
  int _i_sub;
  int s_sn_tmp_0;
  int _s;
  merlin_stream_read(&ch_s_1_p[i_sub],(&s_sn_tmp_0));
  _s = s_sn_tmp_0;
  int b_buf_0_sn_tmp_0;
  int c_sn_tmp_0;
  int a_sn_tmp_1;
  merlin_stream_read(&ch_a_1_p[i_sub],(void *)(&a_sn_tmp_1));
  int a0 = a_sn_tmp_1;
...
void msm_port_s_msm_node_0_0(int s,int i_sub)
{
  ...
  int _s;
  s_sp_tmp_0 = s;
  merlin_stream_write(&ch_s_1_p[i_sub],(&s_sp_tmp_0));
  _s = s;
  _s;
}
...
#pragma unroll
  for (_i_sub_1 = 0; _i_sub_1 <= 3; ++_i_sub_1) {
    merlin_stream_init(&ch_a_1_p[_i_sub_1],(unsigned long )1024,(unsigned long )4);
  }
...
#pragma ACCEL interface variable=c stream_prefetch=on max_depth=1024 depth=1024
  ...
  a_sn_tmp_0 > 100?(static_cast < void  >  (0)) : __assert_fail("a[0] > 100","vec_add.cpp",(unsigned int )13,__PRETTY_FUNCTION__);
// Original pramga:  ACCEL PARALLEL FACTOR=4 
...
#pragma ACCEL STREAM_PORT ACCESS_TYPE=read DIM_NUM=2 NODE=msm_node_0 PARALLEL_ITERATOR=1 VARIABLE=b_buf_0
      ...
      merlin_stream_init(&ch_s_1_p[i_sub],1,4);
...
#pragma ACCEL STREAM_NODE PARALLEL_FACTOR=4 PARALLEL_ITERATOR=2
      msm_node_0(b_buf_0,s,i_sub);
      merlin_stream_reset(&ch_s_1_p[i_sub]);
...
#pragma unroll
  for (_i_sub_2 = 0; _i_sub_2 <= 3; ++_i_sub_2) {
    merlin_stream_reset(&ch_a_1_p[_i_sub_2]);
  }
