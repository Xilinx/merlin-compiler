...
#pragma ACCEL PARALLEL
    ...
    for (int j = 0; j < 1024 * 10; j++) {
      c0 += ((int)(pow((double)(a0 * b0 + j), 1.5))) % 7;
    }
    c[i1 * 4 + i] = c0;
  }
}
...
#pragma ACCEL interface variable = a stream_prefetch = off max_depth =         \
    1024 depth = 1024

#pragma ACCEL interface variable = b stream_prefetch = on max_depth =          \
    1024 depth = 1024

#pragma ACCEL interface variable = c stream_prefetch = on max_depth =          \
    1024 depth = 1024
  for (int i = 0; i < 256; i++) {
