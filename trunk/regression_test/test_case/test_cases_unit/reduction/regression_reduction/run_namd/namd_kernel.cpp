...
float my_erfc(float input)
{
  ...
  return left_temp + 100.0f * (erfc_table_right[right_inx] - left_temp) * ((float )(input - (0.2f + ((float )left_inx) / 100.0f)));
}
...
void pe_1(float q_i,float q_j,float r_ij_x,float r_ij_y,float r_ij_z,float r2,float *force_x,float *force_y,float *force_z)
// float *force_x_temp,
// float *force_y_temp,
// float *force_z_temp
{
...
/* 1/r^2 */
  ...
  float f_vdw_j_z;
  r = sqrtf(r2);
  inv_r = 1.00000000000000000000000000000000000000000000000000f / r;
  inv_r2 = inv_r * inv_r;
  a = r * ewald_coef;
  b = my_erfc(a);
...
/* ds/r */
  ...
  if (r2 > ron2) {
    s = (roff2 - r2) * (roff2 - r2) * (roff2 + 2.0f * r2 - 3.0f * ron2) * inv_denom_switch;
   ...
   *force_x = -(f_elec_j_x + f_vdw_j_x);
   *force_y = -(f_elec_j_y + f_vdw_j_y);
   *force_z = -(f_elec_j_z + f_vdw_j_z);
}
...
void sum_reduce_3d(float &xout,float &yout,float &zout,float x,float y,float z)
{
  xout += x;
  yout += y;
  zout += z;
...
// 4 floats * max number of atoms in each cell
    float ha_buffer[500 * 4];
// 4 floats * max number of atoms in each cell
    float ha_vel_buffer[500 * 4];
// float na_bram[NA_BRAM_SIZE];
    float na_bram[512 * 4];
    float na_bram_p[13824 * 4];
    
#pragma  HLS array_partition  variable=ha_buffer cyclic factor = 16 dim=1
    memcpy_wide_bus_read_float(ha_buffer,((ap_uint<512> *)i_cell_pos),(0 + index * 500 * 4) * sizeof(float ),((unsigned long )(i_cell_cnt(index,cell_size) * 4)) * sizeof(float ));
    
#pragma  HLS array_partition  variable=ha_vel_buffer cyclic factor = 16 dim=1
    memcpy_wide_bus_read_float(ha_vel_buffer,((ap_uint<512> *)i_cell_vel),(0 + index * 500 * 4) * sizeof(float ),((unsigned long )(i_cell_cnt(index,cell_size) * 4)) * sizeof(float ));
    for (i = 0; i < 27; i++) {
...
// memcpy into na_bram
    for (i = 0; i < 27; i++) {
// handle boundary
      if (index_x[i] >= 0 && index_x[i] < 10 && index_y[i] >= 0 && index_y[i] < 10 && index_z[i] >= 0 && index_z[i] < 7) {
        unsigned int atom_cnt = (i_cell_cnt(nbr_index[i],cell_size));
// memcpy (na_bram + na_counter*4, i_cell_pos + (nbr_index[i]*500*4),
// i_cell_cnt(nbr_index[i])*4*sizeof(float));
        
#pragma  HLS array_partition  variable=na_bram cyclic factor = 16 dim=1
        memcpy_wide_bus_read_float(na_bram,((ap_uint<512> *)i_cell_pos),(0 + nbr_index[i] * 500 * 4) * sizeof(float ),((unsigned long )(atom_cnt * ((unsigned int )4))) * sizeof(float ));
...
// i_cell_pos[index*500*4 + j*4 + 0];
      pos_x_i = ha_buffer[j * 4 + 0];
// i_cell_pos[index*500*4 + j*4 + 1];
      pos_y_i = ha_buffer[j * 4 + 1];
// i_cell_pos[index*500*4 + j*4 + 2];
      pos_z_i = ha_buffer[j * 4 + 2];
...
// for large version
      for (i = 0; i < neighbour_to_run; i++) {
        
#pragma HLS loop_tripcount max = 159
        if (index_x[i] >= 0 && index_x[i] < 10 && index_y[i] >= 0 && index_y[i] < 10 && index_z[i] >= 0 && index_z[i] < 7) {
          unsigned int nb_cnt = nbr_atom_cnt[i];
          nb_cnt < 184?((void )0) : __assert_fail("nb_cnt < 184","namd_kernel.cpp",2729,__PRETTY_FUNCTION__);
          for (k = 0; k < (nb_cnt + (16 * 2) - 1) / 16 * 2; k++) {
...
#pragma ACCEL PARALLEL COMPLETE
                ...
                float r_ij_y;
                float r_ij_z;
                pos_x_j = na_bram_p[(512 * 4) * i + (new_k * 4) + 0];
                pos_y_j = na_bram_p[(512 * 4) * i + (new_k * 4) + 1];
                pos_z_j = na_bram_p[(512 * 4) * i + (new_k * 4) + 2];
                q_j = na_bram_p[(512 * 4) * i + (new_k * 4) + 3];
                float r2;
                if (pe_0(pos_x_i,pos_y_i,pos_z_i,pos_x_j,pos_y_j,pos_z_j,&r_ij_x,&r_ij_y,&r_ij_z,&r2) == 1) {
                  pe_1(q_i,q_j,r_ij_x,r_ij_y,r_ij_z,r2,&sub_force_x,&sub_force_y,&sub_force_z);
                }
                
#pragma ACCEL reduction
                sum_reduce_3d(force_x_temp,force_y_temp,force_z_temp,sub_force_x,sub_force_y,sub_force_z);
              }
            }
          }
        }
...
// i_cell_vel[index*500*4 + j*4 + 0];
        vel_x = ha_vel_buffer[j * 4 + 0];
// i_cell_vel[index*500*4 + j*4 + 1];
        vel_y = ha_vel_buffer[j * 4 + 1];
// i_cell_vel[index*500*4 + j*4 + 2];
        vel_z = ha_vel_buffer[j * 4 + 2];
// i_cell_vel[index*500*4 + j*4 + 3];
        mass = ha_vel_buffer[j * 4 + 3];
        pe_2(index,j,q_i,q_j,pos_x_i,pos_y_i,pos_z_i,vel_x,vel_y,vel_z,mass,force_x_temp,force_y_temp,force_z_temp,o_cell_pos,o_cell_vel);
      }
// end for loop: index
    }
//    printf("l1 = %d\n", l1_total);
