...
##LZ: 20180411
#
#my @one_path = "$work_dir/$one_case/run.condor_output";
#push (@path, "$work_dir/$one_case/run");
#sub wanted {
#if ( -f $File::Find::name ) {
#	if ( $File::Find::name =~ /\.log$/ ) {
...
#find(\&wanted, @path);
######
    $log_file = "$work_dir/$one_case/run.condor_output/merlin_test.log";
    if (!-e "$log_file"){
	$log_file = "$work_dir/$one_case/run/merlin_test.log";
	}
    if (!-e "$log_file"){        
       $log_file = "$work_dir/$one_case/run.condor_output/merlin_sim.log";
        }
    if (!-e "$log_file"){
	$log_file = "$work_dir/$one_case/run/merlin_sim.log";
	}
    if (-e "$log_file"){
