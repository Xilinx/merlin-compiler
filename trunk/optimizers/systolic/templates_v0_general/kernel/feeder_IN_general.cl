...
//; my $new_block_size_h = $cfg_root->{Loader_H}->{new_block_size};
//;
...
//; my $num_blocks_to_pe_h = 1;
//; for (my $i = 0; $i < @v_block_dim_h; $i++){
//;    $num_blocks_to_pe_h *= $v_block_dim_h[$i];
//; }
//;
//; my @v_block_dim_v = split(',', $block_dim_v);
//; my $block_reuse_num_h = $v_block_dim_v[0];
//;
//; my $num_blocks_to_pe_h = $num_blocks_to_pe_h * $block_reuse_num_h;
//; my $num_blocks_to_pe_h = $num_blocks_to_pe_h + $v_block_dim_h[@v_block_dim_h - 1];
...
//; foreach my $one_dim (@v_buffer_dim_h) {
//;    $buffer_define .= "[".$one_dim."]";
//;    $fb_total_size *= $one_dim;
//;    $fb_write_size *= $one_dim;
//; }
//;
//; my $fb_read_size = 1;
...
//;    $num_first_vectors *= $v_reuse_num_o[$i];
//; }
...
//	int flow_out_idx = 0;
    ...
    bool more_data_to_feed_in                   = true;
...
#define FB_BLOCK_SIZE_IN (`$fb_total_size - $overlap_size`) // not including padding
...
//; my @v_buffer_dim_h = split(',', $buffer_dim_h);
//; for (my $i = @v_buffer_dim_h - 1; $i >=0 ; $i--){
//;         $it_name = "_i".$i."_w";
	int `$it_name` = 0; 
//; }
...
//		bool data_is_mine_to_buffer = (LAST_FEEDER_IN_CHAIN || (!LAST_FEEDER_IN_CHAIN && !more_data_to_forward));
	//	bool data_is_mine_to_buffer = more_to_write_to_buffer;
		...
		{
//; my @v_new_block_size_h = split(',', $new_block_size_h);
//; my $X = 1;
//; my $pe_iter_dim = $cfg_root->{Feeder_H}->{pe_iter_dim};
//; for (my $i = ($pe_iter_dim + 1); $i < @v_new_block_size_h; $i++){
//;    $X *= $v_new_block_size_h[$i];
//; }
//; $X /= $pe_dsize;
//; my $Y = $v_new_block_size_h[$pe_iter_dim];
//;
//; my $pe_iter_coeff = $cfg_root->{Feeder_H}->{pe_iter_coeff};
//; my $pe_dim_no_r2_lb = $cfg_root->{Feeder_H}->{pe_dim_no_r2_lb};
//; my $pe_dim_no_r2_ub = $cfg_root->{Feeder_H}->{pe_dim_no_r2_ub};

		//	int X = (BS_C + K - 1) * BS_I;
			int X = `$X`;
		//	int Y = (R * BS_R + K - 1);
			int Y = `$Y`;
		//	int xy = ((feed_in_idx / X) % Y) - feeder_id * BS_R;
			int xy = ((feed_in_idx / X) % Y) - feeder_id * `$pe_iter_coeff`;

		//	if ((xy >= 0) && (xy < (BS_R + K - 1)))
			if ((xy >= `$pe_dim_no_r2_lb`) && (xy <= `$pe_dim_no_r2_ub`))
		//	if (data_is_mine_to_buffer /*more_to_write_to_buffer*/ && read_channel_success) 
			...
			//	if (feeder_id == 3)
			//	{
			//		printf("feeder_in_idx=%d {", feed_in_idx);
			//		for (int i = 0; i < 8; i++)
			//		{
			//			printf("%.3f ", data[i]);
			//		}
...
//; }
		...
		//		}
				...
				if (write_side_idx == /*FB_BLOCK_SIZE_IN + FB_PAD_SIZE_IN*/ `$fb_write_size`)
				{
...
//;}
			...
			//	_i0_w = 0;
				...
				//; if ($debug_feeder_in eq "on") {
				{
					int total = /*(FB_BLOCK_SIZE_IN + FB_PAD_SIZE_IN)*/ `$fb_write_size` * `$num_blocks_to_pe_h`;
				//	int total = (FB_BLOCK_SIZE_IN + FB_PAD_SIZE_IN) * (cfg_BN_R[0] * cfg_BN_C[0] * cfg_BN_O[0] + 1) * cfg_BN_I[0];
					if (feeder_id == 0)
					{
...
//;    $ld_block_size *= $v_block_size_h[$i];
//; }
//; $ld_block_size /= $pe_dsize;
		if (read_channel_success)
		{
			feed_in_idx++;
			if (feed_in_idx == /*(R * BS_R + K - 1) * (BS_C + K - 1) * BS_I*/ `$ld_block_size`)
			{
				more_data_to_feed_in = false;
				feed_in_idx = 0;
			}
		}

		// feed out
        if (!LAST_FEEDER_IN_CHAIN && read_channel_success)
		{
#if (ALTERA_FPGA_SDK_16_1 == 1)
			...
			//; if ($debug_feeder_in eq "on") {
			{
				int total = /*(R * BS_R + K - 1) * (BS_C + K - 1) * BS_I */ `$ld_block_size` * `$num_blocks_to_pe_h`;
				if (feeder_id == 0) {
            ...
            if (write_sys_arr_channel_success) {
				...
				*/
				...
				//	int total = BS_R * BS_C * BS_O * K * K * BS_I * (cfg_BN_R[0] * cfg_BN_C[0] * cfg_BN_O[0] + 1) * cfg_BN_I[0];
					int total = FB_READ_SIZE_IN * `$num_blocks_to_pe_h`;
					if (feeder_id == 0) {
...
//; }
        ...
        if (!more_to_write_to_buffer && !more_to_feed_to_sysarr && !more_data_to_feed_in)
		{
            more_to_write_to_buffer = true;
            more_to_feed_to_sysarr  = true;
			more_data_to_feed_in = true;
