...
//; my $num_blocks_to_pe_h = $num_blocks_to_pe_h + $v_block_dim_h[@v_block_dim_h - 1];
	...
	int wr_cnt = 0;
	int rd_cnt = 0;
//	int fw_cnt = -1 * (cfg_BN_R[0] * cfg_BN_C[0] * cfg_BN_O[0] + 1) * cfg_BN_I[0];
	int fw_cnt = -1 * `$num_blocks_to_pe_h`;
	//; }
...
//            block_double_buffer[buffer_id_to_write_to][_i0_w][_i1_w][_i2_w] = data;
			...
			*/
			...
			//; if ($debug_feeder_in eq "on") {
			{
				int total = (FB_BLOCK_SIZE_IN + FB_PAD_SIZE_IN) * `$num_blocks_to_pe_h`;
...
//				}
			...
			//; if ($debug_feeder_in eq "on") {
			{
			//	int total = FB_FORWARD_SIZE_IN * (cfg_BN_R[0] * cfg_BN_C[0] * cfg_BN_O[0] + 1) * cfg_BN_I[0];
				int total = FB_FORWARD_SIZE_IN * `$num_blocks_to_pe_h`;
				if (feeder_id == 0) {
