...
//typedef cl_int cl_data_precision;
...
int __merlin_release_command_queue() 
{
	int err;
	int i;
	for (i = 0; i < (NUM_QUEUES_TO_CREATE + 1); i++) 
	{
		err = clReleaseCommandQueue(cmdQueue[i]);
		if (err != CL_SUCCESS) 
		{
...
//        printf("\n===== Host-CPU transferring weight and input feature map to the FPGA device global memory (DDR4) via PCIe ======\n\n");
	...
	d_weight_block_wise = clCreateBuffer(
		...
		CL_MEM_READ_ONLY,
		num_elem_weight_bw * sizeof(cl_data_precision),
		NULL,
		&status); CHECK(status);
