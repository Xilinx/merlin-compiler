...
static void loader_and_feeder_in(
		int layer,
		data_precision *in1,	data_precision *in1_buf,
...
static void loader_and_feeder_weights(
		int layer,
		data_precision *w1, data_precision *w1_buf,
		int o0, int i0)
{
...
static void PE(
		int layer,
		data_precision *in1_buf, data_precision *w1_buf, 
		data_precision *PE_accum_shift_reg, 
...
static void drain_O(int layer, 
		data_precision *PE_accum_shift_reg, data_precision *out0_buf, 
...
static void addr_trans(int layer, 
		data_precision *out0_buf, data_precision *out1_buf,
		int r0, int c0, int o0)
{
//	const unsigned int cfg_BN_R = (cfg_img_row[layer] + R * BS_R - 1) / (R * BS_R);
//	const unsigned int cfg_BN_C = (cfg_img_col[layer] + BS_C - 1) / BS_C;
...
static void relu_sw(int layer, data_precision *out0_buf, data_precision *out1_buf, 
		int r0, int c0, int o0)
{
	for (int r2 = 0; r2 < R; r2++)
	for (int r1 = 0; r1 < BS_R; r1++)
	...
	//	out2_buf[out2_buf_idx] = out1_buf[out1_buf_idx];
		...
		{
		//	out1_buf[out1_buf_idx] = out0_buf[out0_buf_idx];
			out1_buf[out1_buf_idx] = (out0_buf[out0_buf_idx] - 1e-6) > 0 ? out0_buf[out0_buf_idx] : 0;
		}
		else
		{
			out1_buf[out1_buf_idx] = 0;
...
static void pooling_sw(int layer,
		data_precision *out2_buf, data_precision *out3_buf, 
		int r0, int c0, int o0)
{
	/*
...
static void zero_padding_sw_big(int layer,
		data_precision *out2_buf, data_precision *out5, 
		int r0, int c0, int i0)
{
...
static void zero_padding_sw_little(int layer,
		data_precision *out3_buf, data_precision *out5, 
		int r0, int c0, int i0)
{
	const unsigned int cfg_BN_R = (cfg_img_row[layer + 1] + R * BS_R - 1) / (R * BS_R);
	const unsigned int cfg_BN_C = (cfg_img_col[layer + 1] + BS_C - 1) / BS_C;
...
static void write_to_ddr_big(int layer,
		data_precision *out2_buf, data_precision *out3,
		int r0, int c0, int o0)
{
	const unsigned int cfg_BN_R = (cfg_img_row[layer] + R * BS_R - 1) / (R * BS_R);
	const unsigned int cfg_BN_C = (cfg_img_col[layer] + BS_C - 1) / BS_C;
...
static void write_to_ddr_little(int layer,
		data_precision *out3_buf, data_precision *out3,
		int r0, int c0, int o0)
{
...
static void print_out5(int layer, data_precision *out5)
{
...
static void print_out3(int layer, data_precision *out3)
{
	printf("%s\n", __func__);
...
{
	data_precision in1_buf[R * (BS_R + K - 1) * (BS_C + K - 1) * BS_I * I];
	data_precision w1_buf[O * BS_O * K * K * BS_I * I];
	data_precision PE_accum_shift_reg[R * O * BS_R * BS_C * BS_O];
	data_precision out0_buf[R * BS_R * BS_C * BS_O * O];
	data_precision out1_buf[R * BS_R * BS_C * O * BS_O];
	data_precision out2_buf[R * BS_R * BS_C * O * BS_O];
	...
	memcpy(out5, in1, sizeof(data_precision) * num_elem_input_fm_bw_0);
...
//		if (0 == layer)
//		{
//			memcpy(out5, in1, sizeof(data_precision) * num_elem_input_fm_bw);
//		}
		...
		//	relu_sw(layer, out1_buf, out2_buf);
			relu_sw(layer, out0_buf, out1_buf, r0, c0, o0);
			...
			//	pooling_sw(layer, out2_buf, out3_buf, r0, c0, o0);
				pooling_sw(layer, out1_buf, out1_buf, r0, c0, o0);
			}
			...
			//	{
					if ((layer & 1) == 0)
					{
						zero_padding_sw_little(layer, out1_buf, out5_copy, r0, c0, o0);
					}
					else
					{
						zero_padding_sw_little(layer, out1_buf, out5, r0, c0, o0);
					}
			//	}
			//	else
			//	{
			...
			//	{
				//	write_to_ddr_little(layer, out3_buf, out3, r0, c0, o0);
					write_to_ddr_little(layer, out1_buf, out3, r0, c0, o0);
			//	}
			//	else
