...
__kernel void zero_padding_kernel(__global float *restrict out5, __global float *restrict out5_copy, int num_conv) 
{
//	float out5_buf[(R * BS_R + K - 1) * (BS_C + K - 1) * O * BS_O];
...
#pragma unroll 32
				for (int ii = 0; ii < (BS_I * I); ii++) 
				{
					...
					int out5_idx = zero_padding_sw_little_out5_idx(layer + 1, blk_idx_r0, blk_idx_c0, i0, rr_p, cc_p, ii);
			...
			//	for (int ii = 0; ii < (BS_I * I); ii++) 
				...
				while (1)
				{
					...
					//	int out5_idx = (((blk_idx_r0 - 1) * cfg_BN_C + blk_idx_c0 - 1) * cfg_BN_I + i0) * (R * BS_R + K - 1) * (BS_C + K - 1) * BS_I * I + (rr_p * (BS_C + K - 1) + cc_p) * BS_I * I + ii;
						int out5_idx = zero_padding_sw_little_out5_idx(layer + 1,  blk_idx_r0 - 1, blk_idx_c0 - 1, i0, rr_p, cc_p, ii);
			...
			//	for (int ii = 0; ii < (BS_I * I); ii++) 
				...
				while (1)
				{
					...
					//	int out5_idx = (((blk_idx_r0 - 1) * cfg_BN_C + blk_idx_c0) * cfg_BN_I + i0) * (R * BS_R + K - 1) * (BS_C + K - 1) * BS_I * I + (rr_p * (BS_C + K - 1) + cc_p) * BS_I * I + ii;
						int out5_idx = zero_padding_sw_little_out5_idx(layer + 1, blk_idx_r0 - 1, blk_idx_c0, i0, rr_p, cc_p, ii);
			...
			//	for (int ii = 0; ii < (BS_I * I); ii++) 
				...
				while (1)
				{
					...
					//	int out5_idx = ((blk_idx_r0 * cfg_BN_C + blk_idx_c0 - 1) * cfg_BN_I + i0) * (R * BS_R + K - 1) * (BS_C + K - 1) * BS_I * I + (rr_p * (BS_C + K - 1) + cc_p) * BS_I * I + ii;
						int out5_idx = zero_padding_sw_little_out5_idx(layer + 1, blk_idx_r0, blk_idx_c0 - 1, i0, rr_p, cc_p, ii);
					//	out5[out5_idx] = out3_buf[out3_buf_idx];
						out[out5_idx] = out3_buf[out3_buf_idx];
					}
					else
