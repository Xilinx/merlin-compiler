...
void load_W_prepare()
{
	...
	if ((weights = (float*)acl_aligned_malloc(num_elem_weights * sizeof(float))) == NULL) {
		perror("Failed malloc of weights");
	}
	if ((weights_block_wise = (float*)acl_aligned_malloc(num_elem_weights_bw * sizeof(float))) == NULL) {
		perror("Failed malloc of weights_block_wise");
	}

	float *weight = weights, *weight_block_wise = weights_block_wise;
...
//					t, t_addr, t_i, t_j, t_idx, output_fm_c[t], output_fm_opencl[t]);
	...
	//				t, output_fm_c[t], output_fm_opencl[t]);
			if (err < 10) printf("OPENCL-MODEL ERROR: idx=%d exp=%lf, out=%lf\n",
					t, output_fm_c_block_wise[t], output_fm_opencl_block_wise[t]);
			else if (err == 10) printf("...\n");
			err ++;
		}
#endif
