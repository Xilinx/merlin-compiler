...
//	float *out5_copy = (float *)malloc(sizeof(float) * num_elem_input_fm_bw_max);
	if ((out5 = (float*)acl_aligned_malloc(num_elem_input_fm_bw_max * sizeof(float))) == NULL) 
	{
		perror("Failed malloc of out5");
	}
	if ((out5_copy = (float*)acl_aligned_malloc(num_elem_input_fm_bw_max * sizeof(float))) == NULL) 
	{
		perror("Failed malloc of out5_copy");
	}

	memcpy(out5, input_fm_block_wise, sizeof(float) * num_elem_input_fm_bw_0);
...
//        printf("\n===== Host-CPU transferring weights and input feature map to the FPGA device global memory (DDR4) via PCIe ======\n\n");
	...
	d_out5 = clCreateBuffer(
		...
		CL_MEM_READ_ONLY,
		num_elem_input_fm_bw_max * sizeof(cl_float),
		NULL,
		&status); CHECK(status);
	...
	d_weights_block_wise = clCreateBuffer(
		...
		CL_MEM_READ_ONLY,
		num_elem_weights_bw * sizeof(cl_float),
		NULL,
		&status); CHECK(status);
...
FILE *f;
int next_layer = TEST_LAYER + 1;
int num_elem_input_fm_bw_next_layer = cfg_BN_R[next_layer] * cfg_BN_C[next_layer] * cfg_BN_I[next_layer] * (R * BS_R + K - 1) * (BS_C + K - 1) * BS_I * I;

f = fopen("pad_hw.txt", "w");
for (i = 0; i < num_elem_input_fm_bw_next_layer; i++)
{
	if ((TEST_LAYER & 1) == 0)
		fprintf(f, "%.3f\n", out5_copy[i]);
	else
...
fclose(f);
	...
	if ((f1 = fopen("pad_sw.txt", "r")) == NULL)
	{
		printf("Open file %s for reading fails.\n", "pad_sw.txt");
		exit(1);
	}
	if ((f2 = fopen("pad_hw.txt", "r")) == NULL)
	{
		printf("Open file %s for reading fails.\n", "pad_hw.txt");
		exit(1);
	}
	for (int r0 = 0; r0 < cfg_BN_R[next_layer]; r0++)
	for (int c0 = 0; c0 < cfg_BN_C[next_layer]; c0++)
	for (int i0 = 0; i0 < cfg_BN_O[next_layer]; i0++)
	{
		for (int r_p = 0; r_p < R * BS_R + K - 1; r_p++)
		for (int c_q = 0; c_q < BS_C + K - 1; c_q++)
		for (int i12 = 0; i12 < BS_I * I; i12++)
		{
			float out0_sw, out1_hw;
		...
		//	int out_idx = ((r0 * R * BS_R + r_p) * (cfg_BN_C * BS_C + K - 1) + c0 * BS_C + c_q) * cfg_BN_O * BS_I * I + (i0 * BS_I * I + i12);
			int out_idx_bw = ((r0 * cfg_BN_C[next_layer] + c0) * cfg_BN_O[next_layer] + i0) * (R * BS_R + K - 1) * (BS_C + K - 1) * BS_I * I + (r_p * (BS_C + K - 1) + c_q) * BS_I * I + i12;

			fscanf(f1, "%f", &out0_sw);
			fscanf(f2, "%f", &out1_hw);

			if (fabs(out0_sw - out1_hw) > 0.01)
			{
				if (err < 10)
				{
					printf("out_sw[%d]=%.3f, out_hw[%d]=%.3f\n", out_idx_bw, out0_sw, out_idx_bw, out1_hw);
				}
				else if (err == 10)
				{
					printf("...\n");
