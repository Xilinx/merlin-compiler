...
#filename="${cl_file%.*}"
#output_dir=$filename.sim
filename=`echo $cl_file | awk -F "/" '{print $NF}'`
echo $filename
#dir_prefix=`dirname $cl_file`
#echo $dir_prefix
output_dir=${filename%.*}.sim
echo $output_dir
...
flags="-march=emulator -v --fpc --fp-relaxed --opt-arg -nocaching --report --board $board -DEMULATOR $PARAMS"
