...
#pragma unroll
    ...
    while (1) {
        ...
        if (C_local_out_valid) {
			...
			//; if ($debug_pe eq "on") {
			int total = (PE_ROWS - row) * BS_R * BS_C * BS_O * BN_R * BN_C * BN_O;
...
//#if DEBUG_PE
        {
			...
			int vec_idx = acc_idx / (BN_I * K * K * BS_I *  BS_R * BS_C * BS_O);
			int r0 = -1;
			int c0 = -1;
			int o0 = -1;
			int i0 = (acc_idx / (K * K * BS_I * BS_R * BS_C * BS_O)) % BN_I;
			int r2 = row;
			int o2 = col;
			int o1 = acc_idx % BS_O;
			...
			if (vec_idx >= 0 && vec_idx < BN_R * BN_C * BN_O) {
				r0 = vec_idx / (BN_C * BN_O);
				c0 = (vec_idx / BN_O) % BN_C;
				o0 = vec_idx % BN_O;
				pe_idx = (r0 * BN_C * BN_O + c0 * BN_O + o0) * BS_R * BS_C * BS_O + (r1 * BS_C + c1) * BS_O + o1; 
			}
		//	if (row == 0 && col == 0) {
		//		printf("acc_idx=%d\n", acc_idx);
		//	}
