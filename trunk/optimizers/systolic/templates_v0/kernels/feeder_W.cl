...
//; 
//; my $buffer_dim= $cfg_root->{Feeder_V}->{buffer_dim};
...
//; if ($debug_feeder_w eq "on") {
			{
				int total = (FB_BLOCK_SIZE_W + FB_PAD_SIZE_W) * (BN_R * BN_C * BN_O + 1) * BN_I;
				if (feeder_id == 0)
