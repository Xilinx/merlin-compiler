...
#pragma HLS PIPELINE
				...
				//	in_data.pe_data = ifmap[(r0 * BN_I + i0) * (BS_R * R * BS_I) + vector_id_in_block];
					ddr_data = ifmap[(r0 * BN_I + i0) * ((R * BS_R + K1 - 1) * (C * BS_C + K2 - 1) * BS_I) + vector_id_in_block];
...
//					{
//#pragma HLS UNROLL
//						in_data.pe_simd[i2] = ifmap[(r0 * BN_I + i0) * (R * BS_R * BS_I * I) + vector_id_in_block * I + i2];
//					}

				//	int data_id_in_vector = (data_id_in_block % I);

				//	in_data.pe_simd[data_id_in_vector] = ifmap[(r0 * BN_I + i0) * (R * BS_R * BS_I * I) + data_id_in_block];

				//	if (data_id_in_vector == (I - 1))
				//	{
				//		in_data.new_row_col_pair = new_row_col_pair;
				//		ch_data_row_loader_to_first_feeder << in_data;
				//	}
