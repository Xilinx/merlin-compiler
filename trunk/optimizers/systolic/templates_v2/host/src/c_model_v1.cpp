		...
		int r0, int c0, int o0)
{
	...
	{
		int reg_idx = (((r2 * O + o2) * BS_R + r1) * BS_C + c1) * BS_O + o1;
		int out0_idx = (r0 * cfg_BN_C * cfg_BN_O + c0 * cfg_BN_O + o0) * R * BS_R * BS_C * BS_O * O + (r2 * BS_R * BS_C + r1 * BS_C + c1) * BS_O * O + o1 * O + o2;
       ...
       ) 
{
	...
	float *w1 = weights_block_wise;
	float *in1 = input_fm_block_wise;

	for (int layer = 0; layer < NUM_CONV; layer++)
	{

		int num_elem_input_fm_bw = cfg_BN_R[layer] * cfg_BN_C[layer] * cfg_BN_I[layer] * (R * BS_C + K - 1) * (BS_C + K - 1)  * BS_I * I;
		...
		float *out2 = (float *)malloc(num_elem_output_fm_bw * sizeof(float));
		...
		drain_O_partial(layer, PE_accum_shift_reg, out0, r0, c0, o0);
		...
		for (int o0 = 0; o0 < cfg_BN_O; o0++) {
			...
			for (int o1 = 0; o1 < BS_O; o1++) {
				int out0_idx = (r0 * cfg_BN_C * cfg_BN_O + c0 * cfg_BN_O + o0) * R * BS_R * BS_C * BS_O * O + (r2 * BS_R * BS_C + r1 * BS_C + c1) * BS_O * O + o1 * O + o2;
				int out1_idx = (r0 * cfg_BN_C * cfg_BN_O + c0 * cfg_BN_O + o0) * R * BS_R * BS_C * BS_O * O + (r2 * BS_R * BS_C + r1 * BS_C + c1) * O * BS_O + o2 * BS_O + o1;
	...
	//			t_idx_idx[out1_idx] = (r0 * cfg_BN_C * cfg_BN_O + c0 * cfg_BN_O + o0) * BS_R * BS_C * BS_O + (r1 * BS_C + c1) * BS_O + o1;
				...
				t_idx_i[out0_idx] = r2;
				t_idx_j[out0_idx] = o2;
				t_idx_idx[out0_idx] = (r0 * cfg_BN_C * cfg_BN_O + c0 * cfg_BN_O + o0) * BS_R * BS_C * BS_O + (r1 * BS_C + c1) * BS_O + o1;
			}}
		
		// postprocessing (out1->out2, in DDR for partial validation)
		memcpy(out2, out1, sizeof(float) * num_elem_output_fm_bw);
	
		if (layer < NUM_CONV - 1) {
			// zero padding (out2->out3)
			// FIXME: next_elem_input_fm is computed from block wise data layout, then how to generalize it?
			const unsigned int cfg_BN_R_next = (cfg_img_row[layer + 1] + R * BS_R - 1) / (R * BS_R);
			const unsigned int cfg_BN_C_next = (cfg_img_col[layer + 1] + BS_C - 1) / BS_C;
			const unsigned int cfg_BN_O_next = (cfg_out_num[layer + 1] + O - 1) / O;
			const unsigned int cfg_BN_I_next = (cfg_in_num[layer + 1] + BS_I * I - 1) / (BS_I * I);
	
			int next_elem_input_fm_bw = cfg_BN_R_next * cfg_BN_C_next * cfg_BN_I_next * (R * BS_R + K - 1) * (BS_C + K - 1) * BS_I * I;
	
			float *out3 = (float *)malloc(next_elem_input_fm_bw * sizeof(float));
	
			zero_padding_block_wise(layer, out2, out3);
	
	//		for (int idx = 0; idx < /*next_elem_input_fm*/ 10; idx++) {
	//				printf("%f\n", out3[idx]);
	//		}
	
			memcpy(in1, out3, sizeof(float) * next_elem_input_fm_bw);
		...
		//	memcpy(output_fm_block_wise, out0, num_elem_output_fm_bw * sizeof(float));
		}
