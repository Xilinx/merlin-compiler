...
static void relu_sw(int layer, data_precision *out1_buf, data_precision *out2_buf)
{
	for (int r2 = 0; r2 < R; r2++)
	for (int r1 = 0; r1 < BS_R; r1++)
	for (int c1 = 0; c1 < BS_C; c1++)
...
static void zero_padding_sw(int layer,
		data_precision *out4_buf, data_precision *out5_buf, 
		data_precision *out5,
		int r0, int c0, int i0)
{
#if 0
...
static void write_to_ddr(int layer,
		data_precision *out3_buf, data_precision *out4_buf,
		data_precision *out3,
		int r0, int c0, int o0)
{
       ...
       ) 
{
	...
	data_precision out0_buf[R * BS_R * BS_C * O * BS_O];
	data_precision out1_buf[(R * BS_R + K - 1) * (BS_C + K - 1) * O * BS_O];
	...
	for (int layer = 0; layer < NUM_CONV; layer++)
	{
		...
		for (int o0 = 0; o0 < cfg_BN_O; o0++)
		{
			...
			// drain_O (contains an online out0->out1 address transformation)
