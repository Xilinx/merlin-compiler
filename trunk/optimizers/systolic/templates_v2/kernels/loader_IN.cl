...
__kernel void load_IN_and_forward(__global vec_data_precision_t *restrict out5,
		__global vec_data_precision_t *restrict out5_copy, unsigned char num_conv)
{
	...
	//	unsigned char w_num_blocks_in_pxqxi = cfg_BN_O[layer];
		...
		unsigned char layer_id = read_channel_altera(ch_msg_loader_in_ready);
		mem_fence(CLK_CHANNEL_MEM_FENCE);
