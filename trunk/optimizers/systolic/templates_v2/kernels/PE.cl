...
#else
        sum += A[d] * B[d];
//        sum += 0xFF & (0xFF & A[d]) * (0xFF & B[d]);
#endif
    }

   // data_precision accum = sum + (new_row_col_pair ? 0.0f : accum_shift_reg[ACCUM_SHIFT_REG_SIZE-1]);
...
#pragma unroll
		...
		{
            if (row < (SYS_ARRAY_NUM_ROWS-1))
			{         // pass the results from the PE above through the C_out_shift_reg to the PE below (draining down the PE column)
                bool success;
