...
void block_wise_reformat_input(int layer, float *input, float *input_block_wise)
{
...
void block_wise_reformat_weights(int layer, float *weights, float *weights_block_wise)
{
...
void block_wise_reformat_output(int layer, float *output_block_wise, float *output)
{
 ...
 * */
void zero_padding(int layer, float *out, float *out_padded)
{
...
void relu(int layer, float *relu_in, float *relu_out)
{
	int	num_elem_output_fm = cfg_out_num[layer] * cfg_img_row[layer] * cfg_img_col[layer];

	// workaround
	memcpy(relu_out, relu_in, sizeof(float) * num_elem_output_fm);
}

void pooling(int layer, float *pooling_in, float *pooling_out)
{
...
void print_out_padded(int layer, float *out_padded)
{
	printf("%s\n", __func__);
	FILE *f;
...
void print_conv_out(int layer, float *conv_out)
{
	printf("%s\n", __func__);
...
void print_pooling_out(int layer, float *pooling_out)
{
	   ...
	   )
{
	...
	float *out_padded = (float *)malloc(num_elem_input_fm * sizeof(float));
	...
	for (int layer = 0; layer < NUM_CONV; layer++)
	{
		...
		float *in_copy = (float *)malloc(num_elem_input_fm * sizeof(float));
		float *conv_out = (float *)malloc(num_elem_output_fm * sizeof(float));
		float *relu_out = (float *)malloc(num_elem_output_fm * sizeof(float));
		float *pooling_out = (float *)malloc(num_elem_output_fm_scaled * sizeof(float));

		if (0 == layer)
		{
			memcpy(in_copy, in, sizeof(float) * num_elem_input_fm);
		}
		else
		{
			memcpy(in_copy, out_padded, sizeof(float) * num_elem_input_fm);
		}
		...
		if (layer < NUM_CONV - 1)
		{
			...
			out_padded = (float *)realloc(out_padded, sizeof(float) * next_elem_input_fm);
		...
		{
			if (cfg_pooling_size[layer] > 1)
				memcpy(out, pooling_out, sizeof(float) * num_elem_output_fm_scaled);
			else
				memcpy(out, relu_out, sizeof(float) * num_elem_output_fm);
		}
...
void relu_block_wise(int layer, float *relu_in, float *relu_out)
{
	int	num_elem_output_fm_bw = cfg_BN_R[layer] * cfg_BN_C[layer] * cfg_BN_O[layer] * R * BS_R * BS_C * O * BS_O;

	memcpy(relu_out, relu_in, sizeof(float) * num_elem_output_fm_bw);
}

void pooling_block_wise(int layer, float *pooling_in, float *pooling_out)
{
...
//	fflush(stdout);
	...
	float *scaling_buffer = (float *)malloc(sizeof(float) * scaling_buffer_length);
	memset(scaling_buffer, 0, sizeof(float) * scaling_buffer_length);
...
void zero_padding_block_wise(int layer, float *out, float *out_padded)
{
...
void print_out_block_wise_padded(int layer, float *out_block_wise_padded)
{
	printf("%s\n", __func__);
	FILE *f;
...
void print_conv_out_block_wise(int layer, float *conv_out_block_wise)
{
	printf("%s\n", __func__);
...
void print_pooling_out_block_wise(int layer, float *pooling_out_block_wise)
{
	printf("%s\n", __func__);
...
//	unsigned int num_elem_output_fm = cfg_out_num[0] * cfg_img_row[0] * cfg_img_col[0];
	...
	for (int layer = 0; layer < NUM_CONV; layer++) 
	{
		...
		float *conv_out_block_wise = (float *)malloc(num_elem_output_fm_bw * sizeof(float));
		...
		{
			memcpy(in_block_wise, out_block_wise_padded, sizeof(float) * num_elem_input_fm_bw);
		}
	...
	//		float *out_padded_block_wise = (float *)malloc(next_elem_input_fm * sizeof(float));
			out_block_wise_padded = (float *)realloc(out_block_wise_padded, next_elem_input_fm_bw * sizeof(float));
