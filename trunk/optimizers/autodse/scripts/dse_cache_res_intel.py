...
Filename    : optimizers/autodse/scripts/dse_cache_res_intel.py
Description : This is the script for maintaining a cache to
              estimate the resource utilization of Intel AOCL.
Usage       : Internal call
Owner(s)    : Mohan (mohan@falcon-computing.com)
Version     : 12-11-2018 Created (Cody Hao Yu)
              02-06-2019 Initial modeling (Mohan)
"""
import os
import json
import math
...
from util import run_command, try_to_int, try_to_float
import merlin_logger

MLOGGER = merlin_logger.get_logger('DSE_Cache_Res_Intel')
...
class CacheData(CacheDataBase):
    ...
    def __init__(self, cfg_id, filename, bram_usage, lut_memctrl):
        with open(filename, 'r') as skeleton_file:
            ...
            self.topo_id_map = jdata['topo_id_map']
        self.para_factor = 1
    ...
    def create_qor_graph(self, hls_result):
        ...
        res_singleton = hls_result.res_singleton
        name_topoid_map = hls_result.name_topoid_map
        self.print_breakdown(res_singleton)
        self.print_breakdown(res_breakdown)
        self.analyze_usage(self.top_func, None, 'F', res_breakdown, res_singleton, name_topoid_map, 1)
	...
	self.func_name_map.add(top_func)
    ...
    def analyze_usage(self, node_name, parent, _type, res_breakdown, res_singleton, name_topoid_map,
                      para_factor):
        ...
        if _type == 'L':
            func_name = node_name
            invalid_res = True
            if func_name in self.topo_id_map:
                p_topo = self.topo_id_map[func_name]
                for l_name, topo_list in name_topoid_map.items():
                    func_name = l_name if p_topo in topo_list else func_name
            else:
                MLOGGER.info('ERROR: Error can not find loop name in topo_id_map')
                MLOGGER.info(traceback.format_exc())

            if func_name in res_breakdown:
                if not all([res_breakdown[func_name][res] == 0 for res in MODEL_RES_LIST]):
                    invalid_res = False
                if invalid_res :
                    for res in MODEL_RES_LIST:
                        res_breakdown[func_name][res] = -1
        ...
        for sub_name in sub_skeletons:
            ...
            if child_node_type == 'F':
                child_node = self.analyze_usage(
                    sub_func_name, node, child_node_type, res_breakdown, res_singleton, name_topoid_map, 1)
            else:
                child_node = self.analyze_usage(
                    sub_name, node, child_node_type,
                    res_breakdown, res_singleton, name_topoid_map, sub_para_factor)
                #child_node.para_factor = sub_para_factor
...
class DSEResCacheIntel(DSECacheBase):
    """
    A class for caching QoR results
    """

    def __init__(self, debug):
        super(DSEResCacheIntel, self).__init__(
            'DSEResCacheIntel', debug, False)
    ...
    def gen_metadata_action(self, result_db, ref_data, data, mode):
        ...
        DSEIntelResourceModel.derive_usage(data, ref_data, est_util)
        ...
        for idx, res in enumerate(['BRAM', 'DSP', 'LUT']):
            ...
            old_util = try_to_float(kernel_blk['util-{0}'.format(res)])
            if old_usage is None:
                old_usage = 0
            if old_util is None:
                old_util = 0
            old_util = 0.01 if old_util == 0 else old_util / 100.0
            ...
            if old_usage > 0:
                ...
                new_util = 1
            kernel_blk['total-{0}'.format(res)] = u'{usage}'.format(usage=new_usage)
            kernel_blk['util-{0}'.format(res)] = u'{util}'.format(util=new_util)
...
class DSEIntelResourceModel(object):
    """
    A static class for running Intel resource estimation model
    """
    ...
    def cal_delta_usage(this, other, top_func, para_factor, res):
        ...
        for ele in self_node.child_nodes:
            if ele.type == 'L':
                ...
                sub_para = 1
            sub_delta = DSEIntelResourceModel.cal_delta_usage(
    ...
    def cal_usage(this, other, top_func, para_factor, res):
        ...
        for ele in self_node.child_nodes:
            if ele.type == 'L':
                ...
                sub_para = 1
            sub_usage, sub_changed = DSEIntelResourceModel.cal_usage(
                this, other, ele.name, sub_para, res)
        ...
        if self_node.type == 'L' and para_factor != self_node.para:
            changed = True
        if res_usage >= 0:
            overall_usage = try_to_int(res_usage * para_factor/self_node.para)
        elif not changed and self_node.overall[res] >= 0:
            overall_usage = self_node.overall[res]
        else:
            overall_usage = -1
        MLOGGER.debug("   %s: %s, %s", top_func, overall_usage, changed)
    ...
    def derive_usage(data, ref_data, est_util):
        ...
        est_util.usages[1], _ = DSEIntelResourceModel.cal_usage(
            ref_data, data, ref_data.top_func, 1, 'DSP')
        if est_util.usages[1] < 0:
            est_util.usages[1] = DSEIntelResourceModel.cal_delta_usage(
                ref_data, data, ref_data.top_func, 1, 'DSP')
            if est_util.usages[1] >= 0:
                est_util.usages[1] += \
                    ref_data.qor_graph[ref_data.top_func].overall['DSP']
        MLOGGER.debug("Begin printing usages for LUT:")
        est_util.usages[2], _ = DSEIntelResourceModel.cal_usage(
            ref_data, data, ref_data.top_func, 1, 'LUT')
        if est_util.usages[2] < 0:
            est_util.usages[2] = DSEIntelResourceModel.cal_delta_usage(
                ref_data, data, ref_data.top_func, 1, 'LUT')
            if est_util.usages[2] >= 0:
                est_util.usages[2] += \
