...
#ifdef __cplusplus
template <typename T>
static void memcpy_wide_bus(read, struct, SIZE_1, SIZE_2, LARGE_BUS)(
    T a_buf[][SIZE_1][SIZE_2], size_t index3_offset, size_t index2_offset,
...
#pragma HLS unroll
      ...
      memcpy_read((a_buf[buffer_index / SIZE][(buffer_index % SIZE) / SIZE_2]
...
#pragma HLS unroll
          ...
          memcpy_read((a_buf[index3][index2][index1 * num_elements + j]),
...
#pragma HLS unroll
          for (j1 = 0; j1 < SIZE_2; ++j1) {
            ...
            memcpy_read(
                ...
                (tmp2(a[i + start], data_width, (j * data_width))), data_width,
                T);
          }
        }
        update_index_2
...
#pragma HLS unroll
          for (j1 = 0; j1 < SIZE_1; ++j1) {
            for (j2 = 0; j2 < SIZE_2; ++j2) {
              ...
              if (i == end - start - 1 && j > tail_align)
                continue;
              memcpy_read((a_buf[index3 * bound0 + j0][j1][j2]),
                          (tmp2(a[i + start], data_width, (j * data_width))),
                          data_width, T);
            }
          }
...
template <typename T>
static void memcpy_wide_bus(write, struct, SIZE_1, SIZE_2, LARGE_BUS)(
    MARS_WIDE_BUS_TYPE *c, T c_buf[][SIZE_1][SIZE_2], size_t index3_offset,
        ...
        for (j0 = 0; j0 < bound0; ++j0) {
          for (j1 = 0; j1 < SIZE_1; ++j1) {
            for (j2 = 0; j2 < SIZE_2; ++j2) {
              j = j0 * SIZE + j1 * SIZE_2 + j2;
              memcpy_write((tmp2(c[i + start], data_width, (j * data_width))),
                           (c_buf[index3 * bound0 + j0][j1][j2]), data_width,
                           T);
            }
          }
        }
