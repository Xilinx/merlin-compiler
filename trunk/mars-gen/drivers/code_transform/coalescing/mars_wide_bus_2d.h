...
#ifdef __cplusplus
template <typename T>
static void memcpy_wide_bus(read, struct, SIZE_1,
                            LARGE_BUS)(T a_buf[][SIZE_1], size_t index2_offset,
...
#pragma HLS unroll
      ...
      memcpy_read((a_buf[buf_index / SIZE_1][buf_index % SIZE_1]),
                  (tmp2(a[start], data_width, (j * data_width))), data_width,
                  T);
    }
    return;
  }
...
#pragma HLS unroll
          if (i == end - start - 1 && j > tail_align)
            continue;
          memcpy_read((a_buf[index2][index1 * num_elements + j]),
        ...
        for (j0 = 0; j0 < bound0; ++j0) {
          for (j1 = 0; j1 < SIZE_1; ++j1) {
            ...
            if (i == end - start - 1 && j > tail_align)
              continue;
            memcpy_read((a_buf[index2 * bound0 + j0][j1]),
                        (tmp2(a[i + start], data_width, (j * data_width))),
                        data_width, T);
          }
        }
...
template <typename T>
static void memcpy_wide_bus(write, struct, SIZE_1,
                            LARGE_BUS)(MARS_WIDE_BUS_TYPE *c, T c_buf[][SIZE_1],
                                       size_t index2_offset,
                                       size_t index1_offset, size_t offset_byte,
                                       size_t size_byte,
                                       const size_t const_buf_size) {
#pragma HLS inline 
  const size_t data_width = sizeof(T);
...
#pragma HLS unroll
      ...
      memcpy_write((tmp2(c[start], data_width, (j * data_width))),
                   (c_buf[buf_index / SIZE_1][buf_index % SIZE_1]), data_width,
                   T);
    }
    return;
  }
...
#pragma HLS unroll
          memcpy_write((tmp2(c[i + start], data_width, (j * data_width))),
                       c_buf[index2][index1 * num_elements + j], data_width, T);
        }
        update_index
      } else if (aligned == -1) {
        int j0, j1;
        for (j0 = 0; j0 < bound0; ++j0) {
          for (j1 = 0; j1 < SIZE_1; ++j1) {
            j = j0 * SIZE_1 + j1;
            memcpy_write((tmp2(c[i + start], data_width, (j * data_width))),
                         c_buf[index2 * bound0 + j0][j1], data_width, T);
          }
        }
        index2++;
      }
...
#pragma HLS unroll
      ...
      memcpy_write((tmp2(c[end - 1], data_width, (j * data_width))),
                   (c_buf[(pos + j) / SIZE_1][(pos + j) % SIZE_1]), data_width,
                   T);
    }
  }
}
